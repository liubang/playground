...
BloomFilter::BloomFilter(const uint64_t bit_count)
{
  ...
  while (actual_bit_count < bit_count) { actual_bit_count <<= 1; }

  assert((actual_bit_count & 0x7) == 0);
  assert((actual_bit_count > 0) && (actual_bit_count & (actual_bit_count - 1)) == 0);
...
bool BloomFilter::contains(const void* const data, uint64_t length) const
{
  ...
  for (uint32_t i = 0; i < header_.hash_count; ++i) {
    ...
    seed = hasher.end();
    if (!get_bit(seed % header_.bit_count)) { return false; }
  }
  return true;
}
