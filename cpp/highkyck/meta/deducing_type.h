...
static void test12()
{
  ...
  // 指针在类型推导中也被忽略了，所以即使传的是px，T也被推导成了const int，而不是const int*
}
...
//      1. 如果实参是一个引用，则忽略引用部分
//      2. 如果忽略引用之后，实参还是一个const，那就再忽略const，如果它还是volatile，那么也忽略
template<typename T>
void f3(T param)
{}
...
//      3. 对于传值类型推导，const和volatile实参会被认为是non-const和non-volatile的
//      4. 在模板类型推导时，数组名和函数名实参会退化成指针，除非他们被用于初始化引用
