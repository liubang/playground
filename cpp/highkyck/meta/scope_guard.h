...
#include "highkyck/meta/common.h"
...
class ScopeGuardImplBase {
 ...
 protected:
  ...
  static ScopeGuardImplBase make_empty_Scope_guard() noexcept {
...
class ScopeGuardImpl : public ScopeGuardImplBase {
 public:
  explicit ScopeGuardImpl(FunctionType& fn) noexcept(
      ...
      : ScopeGuardImpl(
            as_const(fn),
            make_failsafe(std::is_nothrow_copy_constructible<FunctionType>{},
                          &fn)) {}
  ...
  explicit ScopeGuardImpl(FunctionType&& fn) noexcept(
      ...
      : ScopeGuardImpl(
            ...
            make_failsafe(std::is_nothrow_move_constructible<FunctionType>{},
                          &fn)) {}
 ...
 private:
  static ScopeGuardImplBase make_failsafe(std::true_type,
                                          const void*) noexcept {
  ...
  explicit ScopeGuardImpl(Fn&& fn, ScopeGuardImplBase&& failsafe)
      : ScopeGuardImplBase{}, function_(std::forward<FunctionType>(fn)) {
    failsafe.dismiss();
  }
