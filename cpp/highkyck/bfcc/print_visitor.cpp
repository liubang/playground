...
{
  for (auto &s : node->Funcs()) { s->Accept(this); }
  sstream_ << "\n";
}
...
void PrintVisitor::VisitorFunctionNode(FunctionNode *node)
{
  ...
  sstream_ << ") {";
  for (const auto &stmt : node->Stmts()) { stmt->Accept(this); }
  sstream_ << "}";
}
...
void PrintVisitor::VisitorForStmtNode(ForStmtNode *node)
{
  ...
  sstream_ << ";";
  if (node->Inc() != nullptr) { node->Inc()->Accept(this); }
  sstream_ << ") ";
  node->Stmt()->Accept(this);
}
...
void PrintVisitor::VisitorConstantNode(ConstantNode *node) { sstream_ << node->Value(); }
