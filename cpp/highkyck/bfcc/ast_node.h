...
struct Identifier
{
  ...
  int64_t offset;
  Identifier(std::string_view name, int64_t offset) : name(name), offset(offset) {}
};
...
public:
  ...
  ProgramNode(std::list<std::shared_ptr<AstNode>> funcs) : funcs_(std::move(funcs)) {}

  ~ProgramNode() override = default;
  void Accept(AstVisitor *visitor) override;
  void PushFunc(const std::shared_ptr<AstNode> &func) { funcs_.push_back(func); }
  [[nodiscard]] const std::list<std::shared_ptr<AstNode>> &Funcs() const { return funcs_; }
...
public:
  ...
  void Accept(AstVisitor *visitor) override;
  void AddParams(const std::shared_ptr<Identifier> &param) { params_.push_back(param); }
  void PushLocalId(const std::shared_ptr<Identifier> &id) { local_ids_.push_back(id); }
  void PushStmt(const std::shared_ptr<AstNode> &stmt) { stmts_.push_back(stmt); }
  [[nodiscard]] std::string_view Name() const { return name_; }
  [[nodiscard]] const std::vector<std::shared_ptr<Identifier>> &Params() const { return params_; }
  [[nodiscard]] const std::list<std::shared_ptr<Identifier>> &LocalIds() const { return local_ids_; }
  [[nodiscard]] const std::list<std::shared_ptr<AstNode>> &Stmts() const { return stmts_; }
...
{
public:
  IfStmtNode(std::shared_ptr<AstNode> c, std::shared_ptr<AstNode> t, std::shared_ptr<AstNode> e)
...
{
public:
  WhileStmtNode(std::shared_ptr<AstNode> c, std::shared_ptr<AstNode> t) : cond_(std::move(c)), then_(std::move(t)) {}
...
{
public:
  DoWhileStmtNode(std::shared_ptr<AstNode> s, std::shared_ptr<AstNode> c) : stmt_(std::move(s)), cond_(std::move(c)) {}
...
public:
  ForStmtNode(std::shared_ptr<AstNode> init = nullptr,
    ...
    : init_(std::move(init)), cond_(std::move(cond)), inc_(std::move(inc)), stmt_(std::move(stmt))
  {}
...
{
public:
  BinaryNode(BinaryOperator op, std::shared_ptr<AstNode> lhs, std::shared_ptr<AstNode> rhs)
...
{
public:
  FuncCallNode(std::string_view func_name, std::vector<std::shared_ptr<AstNode>> args)
  ...
  [[nodiscard]] const std::vector<std::shared_ptr<AstNode>> &Args() const { return args_; }
