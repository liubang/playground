...
int main(int argc, char* argv[]) {
  ...
  io_uring_prep_provide_buffers(
      sqe, test::liburing::bufs, test::liburing::MAX_MESSAGE_LEN,
  ...
  test::liburing::add_accept(&ring, sock_listen_fd, (sockaddr*)&client_addr,
                             &client_len, 0);
      ...
      if (cqe->res == -ENOBUFS) {
        ...
        if (sock_conn_fd >= 0) {
          test::liburing::add_socket_read(
              &ring, sock_conn_fd, test::liburing::group_id,
              test::liburing::MAX_MESSAGE_LEN, IOSQE_BUFFER_SELECT);
        }
        test::liburing::add_accept(&ring, sock_listen_fd,
        ...
        if (cqe->res <= 0) {
          ...
          test::liburing::add_socket_write(&ring, conn_i.fd, bid, bytes_read,
                                           0);
        }
      } else if (type == test::liburing::Status::WRITE) {
        test::liburing::add_provide_buf(&ring, conn_i.bid,
                                        test::liburing::group_id);
        test::liburing::add_socket_read(
            &ring, conn_i.fd, test::liburing::group_id,
            test::liburing::MAX_MESSAGE_LEN, IOSQE_BUFFER_SELECT);
      }
    }
