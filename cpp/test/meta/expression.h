...
template <
    typename T,
    typename U,
    typename = highkyck::meta::enable_if_t<highkyck::meta::is_container_v<T> &&
                                           highkyck::meta::is_container_v<U>>>
auto operator+(const T& lhs, const U& rhs) {
  auto plus = [](auto x, auto y) { return x + y; };
  return highkyck::meta::BinaryContainerExpression(lhs, rhs, plus);
}
