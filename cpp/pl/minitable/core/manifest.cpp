...
absl::StatusOr<std::shared_ptr<const ManifestRef>> DecodeManifest(
    ...
    try {
        ...
        if (!format_version.ok() || !generation.ok() || !next_sequence.ok() || !group_count.ok() ||
        ...
        for (uint32_t i = 0; i < *group_count; ++i) {
            ...
            for (uint32_t j = 0; j < *sst_count; ++j) {
                ...
                if (!sequence.ok() || !key_path.ok() || !value_path.ok() || !key_file.ok() ||
                    !value_file.ok() || !row_count.ok() || !sst_format.ok() || !key_format.ok() ||
                    !schema_fingerprint.ok() || !partition_mode.ok() || !hash_algorithm.ok() ||
                    !virtual_buckets.ok() || !domain_fingerprint.ok() || !checksum_algorithm.ok()) {
                ...
                group.ssts.push_back(
                    {.sequence = *sequence,
                     .identity = {
                         ...
                         .comparator_domain = {.key_format_version = *key_format,
