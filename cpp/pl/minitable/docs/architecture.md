...
截至 2026-07-18，Phase 0 底层硬门槛已形成可测试闭环，Phase 1A 已完成。原 Phase 1B 的 Slice braft/Snapshot 原型也已完成真实三副本选主、复制、切主、Snapshot repair 与 tail log 追赶验证；该实现现在作为确定性状态机和迁移对照基线保留，不再是 Slice 的目标生产复制协议。目标架构调整为：Master 继续使用 braft 管理强一致控制面；Slice 使用通用 `MiniVessel` 框架，以 minidfs 上的共享 WAL、Primary/Standby tail apply、writer epoch fencing、租约和 Checkpoint 实现 HA，SST/Manifest 同样持久化在 minidfs。`SliceApplyMachine`、幂等响应、timestamp watermark、`SliceStore` 和 FileIdentity-fenced Manifest/SST 继续复用。当前 minidfs 已适合不可变 SST，但还不具备 Slice WAL 所需的 active streaming append、durable boundary、tail durable prefix、DataNode writer-epoch fencing 和 lease recovery；因此 Slice braft 在这些能力及 MiniVessel 故障矩阵完成前不得删除或切换权威。后续修改必须同步更新本文的“当前状态”“剩余工作”和分阶段完成定义，避免目标设计与实现状态混淆。
...
- Master 使用 braft；每个 Slice 由 MiniVessel 管理 Primary/Standby，并以 minidfs shared WAL 提供强一致写和可配置的弱一致读；
- 在线 Split、Merge、副本迁移、故障修复和自动 Balance；
- 通过 sstv2、minidfs、MiniVessel、braft（Master）和 brpc 构成自研、可协同演进的完整存储栈。
...
- mutation 已追加到 minidfs shared WAL 的 durable boundary，并由当前持有有效 writer epoch 的 Primary apply 到接口承诺的可见 LSN；
- 返回的 commit timestamp、幂等结果和 payload 已进入可恢复状态；
- Flush/Checkpoint/Compaction 等后台成功不改变已经 ACK 的逻辑数据；
- strong read 的线性化点由当前 assignment/writer epoch、Primary barrier 和 applied LSN 共同证明；
- snapshot read 在固定 `read_ts` 和有效 snapshot lease 下不漏读、不重复、不切换视图；
- 少数派 DataNode、UnitServer 进程或单节点故障不得丢失已 ACK 写；无法证明唯一 writer 或无法达到 minidfs durability policy 时停止写而非不安全降级。
...
1. 已 durable 的 Slice WAL record 必须可确定性重放，apply 阶段不得依赖本地时钟、随机数、角色或本地调度结果。
...
- minidfs 提供共享 WAL、不可变文件的可靠存储、原子发布、校验、租约、writer epoch fencing 和批量 IO；
- MiniVessel 提供 Slice 的 Primary/Standby 角色管理、续租、WAL tail/apply、Checkpoint 和恢复编排；
- braft 仅提供 Master 的复制日志、Leader 选举和配置变更；
...
│ Slice 10 Primary  │       │ Slice 10 Standby  │       │ Slice 10 Standby  │
│ Slice 11 Standby  │       │ Slice 11 Primary  │       │ Slice 11 Standby  │
└─────────┬─────────┘       └─────────┬─────────┘       └─────────┬─────────┘
          │                           │                           │
          └───────────────────────────┼───────────────────────────┘
                                      │ shared WAL tail / immutable SST / checkpoint
                              ┌───────▼────────┐
                              │    minidfs     │
                              └────────────────┘
```
...
| Slice | Table 的一个水平分片和一个独立 MiniVessel group |
| Replica | Slice 在一个 UnitServer 上的 Primary 或 Standby 副本 |
...
采用单 MiniVessel apply writer、多 reader 模型：
...
每个 LG 的 MiniVessel-applied `manifest_generation` 是唯一真相。Manifest 对象文件不可变；本地或 minidfs 的 `CURRENT` 只是不具权威性的加速指针，可丢失、可重建。Flush/Compaction 先完成 SST 和 Manifest 对象，再向 shared WAL 追加 `ManifestEdit`；只有该 record durable 并 apply 后 SST 才进入读路径。恢复时若 `CURRENT` 与 Checkpoint/WAL generation 冲突，以 committed Checkpoint + WAL 为准并重写 CURRENT，禁止形成双真相。
...
5. 原子 finalize 文件；
6. 通过 Slice shared WAL 提交 ManifestEdit；
7. durable 并 apply 后发布新 Version；
8. 满足所有 LG flush watermark、Checkpoint commit 和 replica/backup retention fence 后才允许回收对应 sealed WAL segment；
...
- Merge 链折叠；
- 大小和层级整形；
- 生成新 SST 并通过 shared WAL ManifestEdit 原子替换输入文件。

选择策略综合 size ratio、overlap、read amplification、tombstone ratio、merge debt 和热点。Compaction 不得阻塞 WAL apply，并受全局 IO scheduler 限流。
...
- minidfs shared WAL 是 mutation、ManifestEdit、Primary barrier 和 Checkpoint commit 的权威有序日志；
- ACK mutation 必须不超过 minidfs 发布的 durable LSN，Standby 只能 tail durable prefix；
- minidfs 中已 finalize 且被 applied Manifest 引用的 SST 是长期数据；
- Checkpoint 保存完整 Slice lineage state，而不是单一 Manifest generation；
- 只有 `checkpoint_lsn <= min(flushed_lsn of all LGs)` 且 Checkpoint commit record 已 durable/apply 时，才可推进 WAL GC 候选水位。
...
SliceCheckpoint {
  ...
  visible_applied_lsn
  per_lg { manifest_generation, flushed_lsn, manifest_object_identity }
...
Checkpoint 发布采用“不可变对象 + WAL commit”协议：先基于固定 LSN 构建并校验全部对象，再由 Primary 追加 `CHECKPOINT_COMMIT(checkpoint identity, checkpoint_lsn)`；只有该 record durable 并 apply 后 Checkpoint 才 authoritative。对象上传成功但没有 commit record 的目录只是 orphan。`safe_ts` 不作为可直接信任的独立值恢复；副本根据 Checkpoint 中的 persistent fences、per-LG watermark 和 replay 后 applied state 重新计算。Checkpoint 引用的每个 Manifest/SST 使用稳定 FileIdentity 并在生命周期内 pin。

## 10.2 恢复

```text
load latest committed Checkpoint
  -> validate format/checksum/group-incarnation/schema/operator/timestamp-domain
  -> open every per-LG ManifestRef and verify referenced FileIdentity
  -> restore dedupe, closed-ts proof, GC and operation fences
  -> tail and replay durable WAL after checkpoint_lsn with strict LSN continuity
  -> rebuild Active MemTables and dedupe tail
  -> recompute visible_applied_lsn and safe_ts
  -> validate route/schema/replica-set fences
  -> atomically publish SliceReadView
  -> become STANDBY_READY
```

候选 Primary 还必须获取新的 minidfs writer epoch、追平 durable end、追加并 apply `PRIMARY_BARRIER`，再向 Master 提交 activation proof；此前不得开放写服务。缺文件、checksum 错误、LSN gap 或 operator 缺失时副本不得进入 Serving，必须回退上一代 Checkpoint 或重建。
...
Master metadata 和调度继续使用 braft。每个 Slice 是独立 MiniVessel group：

- Master braft 提交的 `assignment_epoch` 决定 Primary 候选、membership 和路由；
- minidfs 原子分配的 `writer_epoch` 决定唯一 WAL writer，并由 NameNode、DataNode 服务端共同 fencing；
- 写在 WAL record 到达 durable boundary 且 Primary apply 后成功，不要求 Standby 已 apply；
- strong read 只由已经 apply 当前 `PRIMARY_BARRIER`、持有有效 assignment/writer lease 的 Primary 服务；
- bounded-staleness read 要求 Standby safe time/lag 满足限制；
- eventual read 可读 Standby 已应用状态，但仍必须通过 route、replica-set 和本地 Serving fence。

Standby 只有在以下条件全部满足时才能服务：本地 Slice identity/group incarnation 与请求一致；已应用的 `route_epoch` 和 `replica_set_epoch` 不低于请求；自身仍在当前 replica set；请求 read mode 的 `safe_ts/applied_lsn` 条件成立；未处于 Checkpoint install、tombstoned、fenced 或 corruption 状态。Standby 无法仅凭本地旧状态证明 membership 时必须拒绝或向 Primary/Master 验证，不能以 eventual 语义绕过 fencing。

membership 由 Master operation ledger 串行管理。新副本先安装 committed Checkpoint，再 tail WAL；达到 lag 阈值并由 Master 提交新 `replica_set_epoch` 后才成为可提升 Standby。被移除副本在 apply removal fence 后关闭 Serving，再等待 read lease 和本地引用释放后清理数据。

### 12.1 MiniVessel 框架职责

通用框架模块名确定为 `minivessel`，对外名称为 **MiniVessel**。它不实现 consensus，也不理解 minitable Cell；它编排一个外部强一致 Coordinator 与一个支持服务端 fencing 的共享 WAL：

- `Coordinator`：watch assignment、activation proof、progress report；minitable 使用 Master braft adapter；
- `SharedWal`：writer lease、renew/revoke、append+sync、durable boundary、tail、seal 和 recovery；生产实现使用 minidfs；
- `ApplyLog`：业务确定性 apply、applied state、Checkpoint create/install；
- `HaReplica`：角色状态机、续租 danger window、admission、tail/apply、切主和 shutdown；
- `CheckpointManager`：构建、校验、WAL commit、恢复、pin 和 GC fence。

MiniVessel 自身定义独立的 `FileSystem` 能力契约，而不是让 HA runtime 直接依赖 POSIX 或 `DfsClient`：

- immutable object 面复用 sstv2 `FileSystem`，承载 Checkpoint、Manifest 和 SST，要求 finalized identity、精确随机读和 identity-fenced remove；
- active WAL 面提供 `acquire_writer/renew_writer/append/sync/durable_size/read_durable/seal/release_writer`，并显式表达 writer epoch、expected offset、packet sequence 和 durable boundary；
- `FileSystemCapabilities` 对 immutable object、durable WAL、writer fencing、durable tail、lease recovery 分别声明，runtime 启动 authoritative Primary 前必须 fail-closed 校验全部 WAL 能力；
- `LocalFileSystem` 作为开发、单元测试和故障注入后端，使用 POSIX lock、fsync、持久 epoch/durable-boundary sidecar 实现同一语义；
- `MiniDfsFileSystem` 是生产后端：不可变对象面复用现有 sstv2 MiniDFS adapter；WAL 面只有在 minidfs 完成 Phase 1C 的 streaming/sync/tail/fencing/recovery 后才启用，禁止用 immutable stream 或普通 flush 模拟。

角色状态至少包含 `RECOVERING`、`STANDBY_CATCHING_UP`、`STANDBY_READY`、`PRIMARY_ACQUIRING`、`PRIMARY_RECOVERING`、`PRIMARY_BARRIER`、`PRIMARY_SERVING`、`PRIMARY_DRAINING`、`FENCED`、`FAILED`、`TOMBSTONED`。任意续租进入 danger window 或 epoch mismatch 时先关闭写 admission，再进入 `FENCED`；不能等待旧租约自然过期后仍接受请求。

### 12.2 切主协议

Standby 提升顺序固定为：

1. 观察 Master 已提交的新 `assignment_epoch`；
2. 从 minidfs 获取单调增加的 `writer_epoch`，使旧 writer 的 NameNode/DataNode token 失效；
3. 对 active WAL segment 执行 lease recovery，截断到最大安全完整-record durable boundary；
4. replay 到 durable end；
5. 追加并持久化 `PRIMARY_BARRIER(assignment_epoch, writer_epoch)`；
6. apply barrier，并向 Master 上报 activation proof；
7. Master 发布 ACTIVE route 后进入 `PRIMARY_SERVING`。

优雅切主先由旧 Primary 停止 admission、等待已接收写 durable+applied、写入 handoff barrier、seal segment 并释放 writer lease。故障切主必须等待 minidfs 明确 revoke/expire 并完成 recovery；即使 Master 已选出候选，只要 DFS 无法证明旧 writer 已被 fence，新 Primary 就不能开放写。
...
| route / assignment epoch | Slice 路由、Primary 候选与 membership |
| group incarnation | 删除重建后隔离旧 WAL、Checkpoint 和副本 |
| minidfs writer epoch | 旧 Primary、迟到 append/sync/seal RPC 的服务端 fencing |
| WAL LSN / record hash | Slice 日志顺序、连续性与 corruption 检测 |
...
minidfs 同时是 Slice shared WAL 以及 SST、Manifest、Checkpoint 的可靠存储层。minitable 和 MiniVessel 不应假设当前未明确提供的语义。
...
进入 MiniVessel authoritative Slice 闭环前必须先完成以下 WAL P0 能力：

1. **Active streaming append**：under-construction 文件支持 packetized append、expected offset、packet sequence 和同 payload 幂等重试，不通过 close/finalize 模拟每次 WAL append；
2. **Sync 与 durable boundary**：显式 `sync` 返回单调 durable offset/LSN，保证达到配置副本数的 fsync 与元数据发布，重启后不回退，且只位于完整 WAL batch 边界；
3. **Tail durable prefix**：Standby 可读取 active segment `[offset, durable_end)`，支持长轮询、segment rollover、generation/epoch 校验和 retention pin；
4. **Writer epoch 与 DataNode fencing**：NameNode 原子递增 writer epoch；append/sync/seal/recovery token 携带 epoch，DataNode 记录并拒绝旧 epoch，不能只依赖客户端或 Master 检查；
5. **Lease recovery**：新 writer 收集副本 durable length，选择最大满足 durability policy 的完整-record boundary，truncate、bump generation 并使旧 pipeline 永久失效；
6. **权威 segment catalog**：记录 group incarnation、segment chain、open/sealed 状态、first/last LSN、writer epoch 和 durable boundary，不能以目录 `list` 作为连续性真相；
7. **NameNode HA/fencing**：多 NameNode 不得并发签发 writer token，epoch 与 durable boundary 在 failover 后不回退。
...
10. **Staging orphan GC**：过期未 complete 的 minitable staging 文件默认回收而非伪造 checksum 发布；补齐周期 trash purge 和 deleted metadata 清理；
11. **高效范围读取**：multi-range/vectored/async read、cancellation 和 read-ahead；
12. **批量元数据**：batch stat/open/delete 和分页 list；
13. **Checkpoint/bootstrap**：并行下载、校验、断点续传、限速；clone/reference 为可选优化；
14. **资源治理**：rack-aware placement、QoS/backpressure、监控和修复 SLA；
15. **安全**：ACL/服务身份、TLS、namespace 隔离、审计和配额。
...
- metadata HA failover 不产生双 NameNode writer authority，writer epoch 与 durable boundary 不回退；
- 新 writer epoch 生效后，所有 DataNode 拒绝旧 writer 的 append/sync/seal，任意 pause、分区和迟到 RPC 均不能形成双写；
- Standby 永不读取 durable boundary 之外的数据，active segment recovery 后 WAL 无 gap、无重复、无半 record；
...
- Slice WAL append/sync/apply latency、assignment/writer epoch、Primary change、durable/apply lag；
...
1. 同一 Slice 任意时刻最多一个持有有效 assignment 与 minidfs writer epoch、可提交写的 Primary；
2. 每个 key 在任一 route epoch 只属于一个可写 Slice；
3. schema version、route epoch、registration epoch、manifest generation 单调不回退；
4. durable Slice WAL record 在所有副本产生相同状态；
5. 单行 mutation 跨 LG 全可见或全不可见；
6. Checkpoint LSN 不超过所有 LG flush watermark，WAL GC 不超过 committed Checkpoint 与全部 retention fence；
...
21. writer epoch 单调递增，旧 epoch 的 NameNode/DataNode 写请求永远被拒绝；
22. Standby 只 apply durable prefix，LSN 必须严格连续且 writer epoch 不回退；
23. Primary 只有在获取 writer epoch、追平 durable end、durable+apply Primary barrier 并获得 Master activation 后才可 Serving；
24. Checkpoint 只有对应 `CHECKPOINT_COMMIT` record durable+apply 后才 authoritative。
...
在 writer acquire/renew/revoke、WAL append/replicate/sync/durable-boundary/tail/apply、active segment recovery、SST upload/finalize、Manifest CAS、Checkpoint build/commit/install、timestamp domain fence、CF Rewrite、route cutover、Standby promote 和文件 GC 等每个边界注入 crash、timeout、重复和乱序。
...
### Phase 1B：Slice braft 对照基线

当前状态：**原型已完成，冻结为迁移对照，不作为目标生产协议。** `SliceApplyMachine` 的确定性 apply、完整 dedupe response、commit physical time、Snapshot 导入导出以及真实 braft 三副本选主/切主/repair E2E 均已验证。后续只修复阻塞迁移或暴露状态机语义的问题，不继续扩展 Slice braft 产品能力；Master braft 不受此决定影响。

保留该基线的目的：

1. 为 `SliceApplyMachine` 抽取 backend-neutral `ApplyLog` 提供行为 golden；
2. 用同一 mutation sequence 对比 braft replay 与 MiniVessel replay 的逻辑状态、响应和 watermark；
3. 在 MiniVessel 切换权威前提供安全回滚窗口；
4. 禁止在 minidfs P0、MiniVessel 故障矩阵和 canary cutover 完成前删除现有代码。

### Phase 1C：minidfs WAL P0

1. active streaming append、packet identity/expected offset 和幂等重试；
2. replicated `sync`、单调 durable boundary 和 active durable-prefix tail；
3. 单调 writer epoch、NameNode/DataNode token fencing 和保守续租 deadline；
4. lease revoke/recovery、完整-record truncate、block generation bump；
5. authoritative segment catalog、rollover、tail retention pin 和 NameNode HA fence；
6. 独立 WAL fault harness 覆盖 append/ack/fsync/publish/recovery 每个 crash 点。

退出条件：已返回 durable 的 byte/record 在任意单点崩溃后不丢；新 epoch 后旧 writer 在所有 DataNode 被拒绝；Standby 永不读取 durable boundary 之外的数据；active segment recovery 无 gap、重复和半 record。

### Phase 1D：通用 MiniVessel 框架

1. 在 `cpp/pl/minivessel/` 建立 backend-neutral `Coordinator`、`SharedWal`、`ApplyLog`、`CheckpointStore` 接口和强类型 epoch/LSN；
2. 实现 `HaReplica` 角色状态机、writer acquire/renew danger window、admission、tail/apply 和 fencing；
3. 实现 framed WAL codec、CRC、严格 LSN 连续性、Primary barrier 和 proposal tracker；
4. 实现 Checkpoint build/verify/commit/install、retention fence 和 orphan/GC 协议；
5. 定义 capability-driven `FileSystem`，实现 LocalFileSystem 的完整 WAL/object 语义和 MiniDfsFileSystem adapter；MiniDFS WAL P0 未完成前 adapter 必须 fail closed；
6. 提供 deterministic fault injection，并先以最小 KV 状态机验证选主、续租、切主、恢复与 Checkpoint，不与 minitable 业务耦合。

退出条件：框架测试证明旧 writer fencing、durable ACK、Standby tail、优雅/故障切主、Checkpoint 恢复和 GC 安全；业务 apply 代码不依赖角色、时钟或 minidfs RPC。

### Phase 1E：Slice 适配、Shadow 与切换

1. 将 `SliceApplyMachine` 适配为 MiniVessel `ApplyLog`，以 LSN 替换 Raft-specific index 命名；
2. `SliceProposalFrontend` 的 braft submit/term fence 替换为 WAL durable append 和 assignment/writer epoch fence；
3. 将 `SliceSnapshot` 演进为 committed Checkpoint，复用 Manifest/SST FileIdentity 和 dedupe state；
4. 先做 braft authoritative + MiniVessel shadow tail/apply，对比状态 hash、响应和 watermark；
5. canary 阶段通过短暂停写 Checkpoint fence，为 Slice 创建新 group incarnation，Master 原子切换 `replication_mode` 与 route epoch；
6. MiniVessel 产生独有写入后，回退只能做反向 Checkpoint migration，不能直接改配置；
7. 完成多 Slice soak 后删除 Slice braft adapter，Master 继续使用 braft。

退出条件：单 Slice Primary/Standby 切主、重启和 Checkpoint repair 后与 reference model 及 braft golden 等价；已 ACK 写不丢失；所有 orphan 可安全回收；跨 LG 不出现部分可见。

Phase 1 不要求跨 Slice snapshot、在线 Split/Merge、反向 Scan 或完整 Compaction；但 authoritative MiniVessel 上线前要求生产级 NameNode writer fencing，不能以单 NameNode 演示替代。
