...
| **Replica**       | Slice 的一个副本，角色为 Primary (可写) 或 Secondary (可读)；Primary 对应 braft Leader |
...
Cluster
 ...
 │    │    │    ├── Replica 0 (Primary)   ← 主副本, braft Leader, 负责写
 │    │    │    ├── Replica 1 (Secondary) ← 从副本, 可配读
 │    │    │    ├── Replica 2 (Secondary)
...
客户端支持 SQL 或 JSON 描述表结构，转换为 `CreateTableRequest` protobuf 后发往 Master。
Master **不解析 SQL**，只接收结构化 proto：
...
```protobuf
// ...客户端转换为结构化请求发往 Master
message CreateTableRequest {
    string namespace = 1;          // "app_db"
    string table_name = 2;         // "user_profile"
    string region = 3;             // "prod"
    TableSchema schema = 4;        // 见 §7.3
    PartitionSpec partition = 5;   // HASH, 16 slices
    uint32 replicas = 6;           // 3
}
```
...
ShardManager.get("app_db", "user_profile", RowKey("acme", 42))
  ...
  cache[("app_db", "user_profile", 3)] → {
      slice_id: 1003,
      replicas: [
          {role: PRIMARY,    host: "us-0:8100"},
          {role: SECONDARY,  host: "us-1:8100"},
          {role: SECONDARY,  host: "us-2:8100"},
      ]
  }
```
...
Client                                Master
  │                                      │
  │── 1. GetSlice(ns, table, rowkey) ──────▶│   (缓存命中的话跳过这步)
  │◀──── {SliceId, [Primary, Secondaries]}─┘
  │
  │── 2. Put(slice, mutation) ──brpc──▶ UnitServer (Primary Replica)
  │                                         │
  │                              ┌──────────┤
  │                              │          ▼
  │                              │    不是 Primary/路由过期?
  ...
  │                              │    ┌─────┴─────────────┐
  │                              │    ▼                   ▼
  │                              │ NOT_PRIMARY       SLICE_MOVED
  │                              │ (携带新Primary地址) (Split/迁移)
  │                              │    │                   │
  │                              │    └──────┬────────────┘
  │                              │           │
  │◀───────── 错误码 ◀────────────┘          │
  │                                          │
  │── 3. 刷新路由缓存 ◀── Master ────────────┘
  │── 4. 重试 Put ──▶ 新 Primary / 新 Slice ──▶ ACK
```
...
| 策略            | 行为             | 一致性   |
| --------------- | ---------------- | -------- |
| `READ_PRIMARY`   | 总是读 Primary   | 强一致   |
| `READ_SECONDARY` | 读 Secondary 副本 | 最终一致 |
...
| 错误码             | 触发场景                   | 响应携带                  |
| ------------------ | -------------------------- | ------------------------- |
| `NOT_PRIMARY`      | 请求发到了 Secondary 副本 | `primary_addr` — 新 Primary |
...
Client.put(ns, table, rowkey, mutation):
  ...
  │     ├─ on SUCCESS → return
  │     ├─ on NOT_PRIMARY:
  │     │     ShardManager.update_primary(slice_id, resp.primary_addr)
...
public:
    ...
    void update_primary(SliceId slice_id, const std::string& primary_addr);
};
```
...
Flusher:
  ...
  ├─ 写文件到 minidfs: <region_dfs_root>/<ns>/<table>/<slice>/<lg_name>/sst_<seq>.sstv2
...
| 模块                | 线程模型         | 功能                                                             |
| ------------------- | ---------------- | ---------------------------------------------------------------- |
| **SchemaService**   | 1 线程 + 64 队列 | DDL 编排: schema 解析, 兼容检查, 生成 Slice 计划 → braft::apply  |
| **RegionService**   | 1 线程 + 4096 队列 | Heartbeat 处理, UnitServer 注册/注销, Slice 调度决策             |
| **RouteTable**      | 共享只读 (无锁)  | 维护 `(ns, table) → [Slice key-range → Replica 地址]`, shared_ptr 快照 |
| **HeartbeatMonitor** | 1 后台线程       | 周期检测心跳超时 (10s STALE / 30s DEAD) → 触发 failover         |
| **SnapshotManager** | 1 后台线程       | 周期触发 braft snapshot (默认 10min)                              |

**brpc 线程**: 仅做反序列化 + 参数校验 + dispatch 到 Service 队列，不执行阻塞或 CPU 密集操作。
...
Client                                  Master
  │                                       │
  │  SQL / JSON → CreateTableRequest pb   │
  │                                       │
  │── CreateTable(schema) ──────────────▶│
  │                                       ├─ brpc: 校验 → enqueue SchemaService
  │                                       ├─ SchemaService: 验证, 生成 Slice 计划
  │                                       ├─ SchemaService: braft::apply(task)
  │                                       ├─ on_apply: 更新 ClusterMeta
  │                                       ├─ on_apply: SliceScheduler 分配 Replica
  │                                       ├─ on_apply: RouteTable.publish()
  │                                       ├─ 通知 UnitServer 启动 Slice
  │◀── table_id ─────────────────────────│
```
...
message RegionConfig {
    ...
    int32 max_slices = 2;
    string dfs_root = 3;                   // 该 Region 的 minidfs 根路径
                                           // 默认 "/minitable/data/<region_name>"
}
...
message SliceReplica {
    ...
    ReplicaRole role = 3;      // PRIMARY | SECONDARY | OFFLINE
}

message UnitServerInfo {
    uint64 us_id = 1;          string region = 2;
    string host = 3;           uint32 port = 4;          int64 capacity = 5;
    // 注意: state 和 last_hb_us 不持久化 — 由 RegionService 在内存中维护
    // HeartbeatMonitor 后台线程周期检测超时, 触发 failover
}
```
...
Region 是资源隔离的基本单元，存储布局以 Region 为最外层目录：
...
│
└── data/
    └── <region>/                 ← 资源池, 顶层隔离 (e.g. "prod", "test")
        └── <namespace>/          ← 表级逻辑隔离 (e.g. "app_db", 默认 "")
            └── <table_name>/     ← 表名
                └── <slice_id>/   ← 分片 (e.g. 1003)
                    ├── snapshot_meta.pb  ← Slice braft snapshot
                    └── <lg_name>/        ← LocalityGroup (e.g. "default", "meta")
                        ├── sst_000001.sstv2        ← key file (sstv2 格式)
                        ├── sst_000001_value.sstv2  ← value file
                        └── ...
```
...
| 层级           | 隔离语义                                                 | 生命周期                                         |
| -------------- | -------------------------------------------------------- | ------------------------------------------------ |
| `<region>`     | 资源池物理隔离，不同 Region 可使用不同 minidfs 路径/配额 | 与 Region CRUD 绑定，删除 Region 时级联清理      |
| `<namespace>`  | 表级逻辑隔离，同一 Region 内 namespace 唯一              | 由 Table 的 namespace 字段决定，表删除则目录移除 |
| `<table_name>` | 单表所有数据                                             | CREATE/DROP TABLE                                |
| `<slice_id>`   | 分片级隔离，含 Snapshot + 所有 LG                        | Slice Split/Merge/迁移时创建或删除               |
| `<lg_name>`    | 列族物理隔离，独立 LSM 引擎                              | 与 Table Schema 中的 LG 定义绑定                 |

**Region 与 minidfs 路径映射**:

Region 可配置独立的 minidfs 根路径，实现不同资源池使用不同存储后端或配额：
...
message RegionConfig {
    int32 max_tables = 1;
    int32 max_slices = 2;
    string dfs_root = 3;   // 该 Region 的 minidfs 根路径, 默认 "/minitable/data/<region>"
}
```
...
```cpp
std::string dfs_path(const Region& region, const std::string& ns,
                      const std::string& table, uint64_t slice_id,
                      const std::string& lg_name) {
    return absl::StrCat(region.config.dfs_root(), "/",
                        ns.empty() ? "_default" : ns, "/",
                        table, "/",
                        slice_id, "/",
                        lg_name, "/");
}
...
**与现有 path 引用的一致性**: 6.5 Flush 中的 SSTable 文件路径 `minitable/data/<ns>/<table>/<slice>/<lg_name>/sst_<seq>.sstv2` 需相应更新为 `<region_dfs_root>/<ns>/<table>/<slice>/<lg_name>/sst_<seq>.sstv2`。

### 7.6 RPC 定义

Master 对外提供三组 proto service，分散在三个 proto 文件中：
...
// === unit.proto — Master↔UnitServer 内部通信 ===
service UnitService {
...
message GetSliceRequest {
    string namespace = 1;  string table_name = 2;
    bytes row_key = 3;    // 空 → 全表 Slice 列表
}
message GetSliceResponse {
    int32 error_code = 1;  string error_msg = 2;
    repeated SliceInfo slices = 3;
}
message SliceInfo {
    uint64 slice_id = 1;  bytes start_key = 2;  bytes end_key = 3;
    repeated ReplicaEndpoint replicas = 4;
}
message ReplicaEndpoint {
    uint32 replica_id = 1;  uint64 us_id = 2;
    string host = 3;  uint32 port = 4;
    ReplicaRole role = 5;  // PRIMARY=0, SECONDARY=1, OFFLINE=2
}
```

**CreateTable** — 客户端解析 SQL/JSON 后发送结构化请求:

```protobuf
message CreateTableRequest {
    string namespace = 1;   string table_name = 2;
    string region = 3;      TableSchema schema = 4;
    PartitionSpec partition = 5;
    uint32 replicas = 6;
}
```

Master 不解析 SQL，只做 schema 合法性校验。

**Heartbeat** — fire-and-forget, 不经过 braft:

```
brpc Thread         RegionService Thread      braft Thread
  ├─ enqueue ──────▶├─ update last_hb_us      (不涉及)
  │                 ├─ 检测: DEAD? → braft::apply ──▶ on_apply: failover
```
...
│
├── proto/
│   ├── master.proto              # MasterService (DDL + GetSlice)
│   ├── admin.proto               # AdminService (Region 管理)
│   ├── unit.proto                # UnitService (Heartbeat + US 注册)
│   └── BUILD
│
├── common/
...
│
├── master/
│   ├── master.h / master.cpp     # brpc Service (实现全部 3 个 proto service)
│   ├── master_sm.h / master_sm.cpp  # braft::StateMachine
│   ├── schema_service.h          # DDL 编排 (1 thread + queue)
│   ├── region_service.h          # Heartbeat/US/Slice 调度 (1 thread + queue)
│   ├── route_table.h             # 路由表 (shared_ptr 快照, 无锁读)
│   ├── heartbeat_monitor.h       # 后台心跳超时检测
│   ├── snapshot_manager.h        # 后台快照触发
│   ├── bounded_task_queue.h      # 有界任务队列 (待替换为 ThreadPool)
