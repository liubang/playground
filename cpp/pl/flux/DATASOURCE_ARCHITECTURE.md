...
- Profile：pipeline profile 包含 drivers/pages/rows/blocking/finished/error；connector split
  ...
  decode/page-build 分段耗时。
- Benchmark：已有 SQLite/MySQL scan benchmark target，可用同结构数据集比较本地 SQLite 和远程
  MySQL connector scan 路径；runner 可自动重建 MySQL benchmark 表。2026-05-16 的 1M 行
  `filter_project` 对比见 [benchmark/README.md](./benchmark/README.md)。
...
- 本机真实 SQLite scan benchmark：
  ...
  - 1M rows：8 drivers，500k output rows，496 pages，约 0.0548s - 0.0576s。
- 远程 MySQL 同结构 scan benchmark：
  ...
  - 1M rows：8 drivers，500k output rows，483-488 pages，约 0.195s / 0.280s / 0.670s；
    另一次重建 fixture 后单次确认约 0.187s。
  - 结论：同样 `filter_project` 查询下，远程 MySQL 端到端仍明显慢于本地 SQLite；新的分段 profile
    显示主要成本在远程 read、连接、execution start 和 split discovery，Page build 已经降到
    亚毫秒级，不再是首要瓶颈。
...
- MySQL connector 是否应该引入连接复用和 prepared statement 缓存，以继续减少小查询和多 split
  查询的固定开销？
...
- 连接复用 / prepared statement cache：当前 MySQL 每个 split 仍独立连接与 start execution，下一轮应先
  做 connection pool，再评估 prepared statement 是否值得进入热路径。
- Page source 参数化：把 rows/page、Boost.MySQL read batch 和 split count 变成 connector runtime
  config，方便按局域网/远程库调优。
- Benchmark baseline 刷新：在新的 profile 字段稳定后，持续刷新 SQLite/MySQL 100k/1M/更大规模基线。
