...
- Execution：physical planner 产出 `ExecutionTask` / `Pipeline`，scheduler 通过 `TaskExecutor`
  运行 driver task；operator 之间主通道是 `Page` / `PageChunk` / `ColumnVector`。
- Streaming：scan/filter/project/range 和 root exchange 已经逐 Page 执行；sort/group/distinct/
  aggregate/join/materialize 仍是明确 blocking boundary。
- Profile：pipeline profile 包含 drivers/pages/rows/blocking/finished/error；connector split
  profile 包含 pages/rows/bytes/wall time/finished。
- Benchmark：已有 SQLite/MySQL scan benchmark target，可用同结构数据集比较本地 SQLite 和远程
  MySQL connector scan 路径。2026-05-16 的 1M 行 `filter_project` 对比见
...
先通过 `TableValue` 作为跨层主接口。profile 会汇总 connector split 的 pages/rows/bytes/time/finished
...
contract。`TableValue` 只在旧 builtin、blocking helper 和最终 `Value` 输出边界互转；
...
- `RangeOperator`、`FilterOperator`、`ProjectOperator`、`RenameOperator`、`LimitOperator` 和
  ...
  `TableValue`。`SortOperator`、`GroupOperator`、`DistinctOperator` 和 `AggregateOperator` 是
  明确 blocking boundary，先收集 pages，再复用表语义 helper。
...
- `MySQLPageSource` 支持 split column range，把 split predicate 合入 page source SQL，
  对 executor 暴露的仍是标准 `ConnectorPageSource`。
...
- `ExchangeBuffer` 增加 buffered page/byte budget，producer 在 queue 满或预算不足时阻塞，
  单个超大 page 允许通过，避免死锁。
- connector split profile 增加 bytes 和 wall time，profile/explain formatter 与 benchmark
  都会输出这些字段，用于判断吞吐瓶颈发生在 page source 还是后续 operator。
- `//cpp/pl/flux/benchmark:sqlite_scan_benchmark` 可构造真实 SQLite 数据库并执行
  `scan`、`filter_project`、`wide_filter`、`topn` 等 scenario，输出 rows、scenario、drivers、
  pages、split bytes、split wall time、blocking、seconds 和 rows/s。
...
- 本机真实 SQLite scan benchmark：
  - 100k rows：8 drivers，50k output rows，56 pages，约 0.0087s。
  - 1M rows：8 drivers，500k output rows，496 pages，约 0.056s - 0.072s。
- 远程 MySQL 同结构 scan benchmark：
  - 表：`flux_bench_cpu(_time, host, region, usage, seq primary key)`，数据生成逻辑与 SQLite
    benchmark 一致。
  - 100k rows：8 drivers，50k output rows，52 pages，约 0.121s。
  - 1M rows：8 drivers，500k output rows，488 pages，约 0.256s - 0.259s。
  - 结论：同样 `filter_project` 查询下，远程 MySQL 端到端约慢 `3.5x - 4.5x`；这主要反映
    网络、连接/metadata、MySQL server 执行、协议解码和 row-to-Page 转换成本，不代表 SQL
    connector 主干不可扩展。
...
- MySQL connector 是否应该引入连接复用、metadata/statistics 缓存和 prepared statement 缓存，
  以减少小查询和多 split 查询的固定开销？
...
- MySQL integration test 在当前 Bazel/CI 环境中如何稳定运行，以及是否需要独立 benchmark
  fixture 初始化脚本？
- CBO 的 statistics 来源来自 connector schema、采样、显式 analyze，还是先只做 rule-cost placeholder？
- Page source 的 target page size、batch decode 和 string/value allocation 是否需要进入统一
  runtime config？
...
- MySQL page source 热路径：连接复用、schema/statistics 缓存、range split discovery 缓存、
  row decode 到 `ColumnVector` 的批量化。
- Connector benchmark 固化：把同结构 SQLite/MySQL 数据集生成流程做成可重复脚本，避免手写 SQL。
- Profile 加强：拆分 connector metadata time、split discovery time、page read time 和
  downstream operator time。
