...
`Driver`、独立 `Page`/`PageChunk`/`ColumnVector` 和 typed `Operator`；physical planner 已经能产出同步执行的 pipeline。
...
状态：已启动。当前已新增 logical `JoinSpec` / `ExchangeSpec`、physical
`LocalHashJoin` / `Exchange` 节点、二输入 `LocalHashJoinOperator` 和本地
`ExchangeOperator` placeholder。executor 已能把两个 connector-backed inputs 接到同一个
local hash join；exchange 暂时是单机 page boundary，不伪装成分布式 shuffle。
...
- `StatsProvider` / `CostModel` 已扩展到 join/exchange；connector column statistics contract
  现在包含 NDV、null fraction 和 average width，未知值显式保持 unknown。
- CBO alternatives 不再只有单个 RBO facade：connector full pushdown 会同时记录 memory suffix
  ...
  及 build-side alternative 占位。
- join 先实现 local hash join physical node；跨源 join 不下推，但 executor 已能规划/执行
  多输入 operator tree。
- 新增 `Exchange` logical/physical/operator 占位，为后续并发、repartition 和 pipeline 分离做准备。

已落地：

- `PlanNode` 新增 `MakeJoin`、`MakeExchange`、`JoinMethod`、`ExchangeKind`。
- `PhysicalExecutor` 新增 `LocalHashJoinOperator`，默认 build 右表、probe 左表，支持
  inner/left/right/full 的基础行补齐语义。
- `PhysicalPlanner` 已能构造多输入 operator tree；当前仍放在单个 synchronous pipeline 中执行，
  这是单机执行骨架，不是假分布式。
- `ExchangeOperator` 作为 local page boundary 直接传递 page，并在 plan/explain 中显式出现。
- `CostModel` 能估算 local hash join 和 exchange 的启发式成本；join side 选择目前只记录
  alternative，不在 unknown/弱 cost 下自动换边。

验收：

- physical executor 能执行两个 connector input 的 local hash join。
- physical planner 能展示/返回二输入 join operator tree。
- exchange placeholder 能作为显式 page boundary 执行。
- CBO 未启用或 stats unknown 时，explain 明确显示 fallback 到 RBO。
- 不因为不可靠 cost 自动改变 join 顺序；当前只记录 build-side alternative。
...
下一轮建议进入 Phase 10：在现有 Page-native pipeline 和 connector split/page source 基础上，
补 local hash join physical node、exchange placeholder 和更完整的 CBO alternative 枚举。

这样可以在不引入分布式运行时的前提下，把跨源 join、代价选择和后续并行 split 调度的接口先压实。
