...
pipeline 并合并输出。当前调度是单机同步顺序执行，已具备本地 gather/repartition exchange 语义，
尚未引入并行 driver。
...
- join 先实现 local hash join physical node；跨源 join 不下推，但 executor 已能规划/执行
  多输入 operator tree。
- 新增 `Exchange` logical/physical/operator：gather 会 materialize 成单个本地输出 chunk，
  repartition 会按 partition key 把 rows 分成本地 chunks，为后续并发和远端 exchange 留接口。
...
- exchange gather/repartition 能作为显式 materializing boundary 执行，repartition 后同 key rows
  位于同一个本地 chunk。
...
- `//cpp/pl/flux/benchmark:sqlite_scan_benchmark` 可构造真实 SQLite 数据库并执行
  ...
  pages、blocking、seconds 和 rows/s。
