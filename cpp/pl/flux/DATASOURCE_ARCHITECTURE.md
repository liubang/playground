...
- Streaming：scan/filter/project/range 和 root exchange 已经逐 Page 执行；group/distinct/
  aggregate 是 streaming accumulator，逐 Page 吸收输入并最终产出结果；sort/topn/join/
  materialize 仍是明确 blocking boundary。
- Profile：pipeline profile 包含 drivers/pages/rows/blocking/finished/error；connector split
  ...
  decode/page-build 分段耗时；formatter 会标出 blocking pipeline 内的 accumulator 算子。
