...
private:
    ...
    void add_definition(const std::string& name, SymbolKind kind, const SourceLocation& loc,
                        std::vector<std::string> params = {});
    void add_reference(const std::string& name, const SourceLocation& loc);

    std::string uri_;
    SymbolTable table_;
    // Track defined names in current scope to detect undefined references later
...
void SymbolCollector::visit_file(const File& file) {
    ...
    static const std::unordered_set<std::string> universe_names = {
        "from",    "to",     "range",   "filter", "map",    "limit",  "tail",
        "keep",    "drop",   "rename",  "sort",   "group",  "pivot",  "fill",
        "union",   "count",  "sum",     "mean",   "min",    "max",    "spread",
        "quantile","median", "first",   "last",   "top",    "bottom", "reduce",
        "distinct","window", "aggregateWindow", "elapsed", "difference",
        "derivative","join", "columns", "keys",   "findColumn", "findRecord",
        "explain", "yield",  "true",    "false",  "nothing","die",
        "contains","length", "string",  "int",    "float",  "uint",   "bool",
        "time",    "duration","bytes",   "display","exists",
    };

    for (const auto& ref : table_.references) {
        if (defined_names_.count(ref.name) > 0) {
            continue;
        }
...
void SymbolCollector::add_definition(const std::string& name, SymbolKind kind,
