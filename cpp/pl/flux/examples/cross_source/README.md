...
- `sqlite_pushdown_explain.flux`：SQLite 数据源下推、logical explain 和 physical explain 输出。
- `sqlite_csv_join.flux`：SQLite 指标表 join CSV owner 维表。
- `sqlite_csv_array_incidents.flux`：SQLite 指标、CSV owner 维表和 array 阈值表组合。
- `mysql_pushdown_explain.flux`：MySQL 数据源下推、logical explain 和 physical explain 输出。
