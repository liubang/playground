...
- Streaming：scan/filter/project/range 和 root exchange 已经逐 Page 执行；group/distinct/
  aggregate 是 streaming accumulator，逐 Page 吸收输入并最终产出结果；多 driver 的高基数
  `group |> aggregate`、root `group` 和 root `distinct` 会通过 hash partition exchange 拆成
  partial/final 两阶段，final driver 按 key 分区并行合并；sort/topn/join/materialize 仍是明确
  blocking boundary。
- Profile：pipeline profile 包含 drivers/pages/rows/blocking/finished/error；connector split
  ...
  accumulator phase/key strategy/partial-final 耗时。pipeline plan 和 runtime profile 会暴露
  exchange distribution；query 级 `QueryMemoryContext` 暴露 used/peak/limit/limited。
...
- 当前定位不实现 spill。blocking operator 超过查询内存预算时直接返回 `ResourceExhausted`，等
  query 主干彻底稳定后再重新评估是否需要外部排序/落盘。
...
- 新增本文档，明确多数据源演进方向。
- 暂不改运行时行为。
- 后续每个阶段都同步 README、support-matrix.md 和测试。
...
- `ConnectorMetadata` 暴露轻量 `Statistics()` 接口；SQLite/MySQL 通过聚合 SQL 暴露 row count、
  列级 NDV、null fraction 和 average width，array/csv memory runtime 通过内存行推导同形状统计。
  统计只进入 optimizer/explain 边界，避免污染 page source 执行路径。
...
- `StatsProvider` / `CostModel` 已扩展到 join/exchange；connector column statistics contract
  ...
  无法取得的字段显式保持 unknown。Phase 17 后 CBO 会沿 unary plan 传播列统计，并在
  filter/group/distinct 基数估算中消费 NDV 和 null fraction。
...
- SQL connector pushdown 有统一 contract：在 SQL 生成前集中校验 projection、predicate、
  ...
  statement，SQLite 继续使用已有的本地参数绑定。SQLite rowid split 和 MySQL range split 的
  边界值也统一进入 bind vector，不再为每个 split 生成不同 SQL text。
...
### Phase 16: Cost-aware Partitioned Accumulator, Query Memory, and Structured Runtime Explain

状态：已完成本地 blocking accumulator 的 cost-aware 并行执行形态。planner 不再只要多 split 就
固定生成 partial/global gather，而是结合 CBO 行数估算、group 基数估算和 root wrapper 约束，在
single-stage、two-stage gather、two-stage partitioned final 之间选择。高基数 root
`group |> aggregate`、root `group` 和 root `distinct` 会把 partial 结果按 key 分发到多个
final driver，低行数查询仍保持 single-stage，避免小查询为并行框架付固定成本。

目标：把高效查询主干从“多 split scan + 单 final accumulator”推进到“scan/partial/final 都能并行”，
同时补齐查询级内存 guard、exchange distribution 可观测性、结构化 profile 和 benchmark
regression gate，让性能结论可以被机器化复验。本阶段明确不做 spill；超过查询内存预算直接失败，
避免在主执行链路还在收紧时引入落盘路径。
...
- root `group` / `distinct` 也接入 partitioned partial/final：partial driver 局部去重/分组后
  通过 hash distribution 写入多个 `ExchangeBuffer`，final driver 按同一 key 分区继续合并。
- 新增 exchange distribution profile：pipeline/execution explain 会显示 gather/hash、
  partition keys、是否包含 group key 和 partition 数；可见 operator 名称使用
  `HashPartitionExchangeSinkOperator`。
- accumulator 内存统计升级为 query 级 `QueryMemoryContext`：`FLUX_QUERY_MAX_MEMORY_BYTES`
  控制整条查询预算，兼容读取 `FLUX_ACCUMULATOR_MAX_BYTES`，profile 同时暴露 query 级
  `used/peak/limit/limited` 和 accumulator 局部 `memory_bytes/memory_limit_bytes/memory_limited`。
- `FormatPipelinePlanJson` 和 `FormatExecutionProfileJson` 提供结构化 JSON；`explain(pipeline: true,
  json: true)` 可以直接返回 pipeline DAG JSON，便于 UI 或自动化工具消费；`explain(graph: true)`
  可返回 logical / optimized logical / physical / pipeline 的 Mermaid 图，便于文档和可视化工具直接渲染；
  profile JSON 也包含 query memory 和 per-pipeline distribution。
- connector benchmark runner 增加 `--compare-baseline` 和 `--regression-threshold`，会按
  connector/scenario 比较 `median_s`，超过阈值时在 JSON 中写入 `regressions` 并返回非 0；SQLite
  和 MySQL benchmark JSON 都会输出 query memory 指标。

验收：

- runtime 覆盖小输入 grouped aggregate 不生成 partial/final，避免小查询退化。
- runtime 覆盖高基数 grouped aggregate 生成 partitioned final，并验证结果正确。
- runtime 覆盖 root `group` / root `distinct` 生成 hash partitioned partial/final，并验证结果正确。
- runtime 覆盖 query memory guard 的 `ResourceExhausted` 错误。
- runtime 覆盖 pipeline/profile JSON formatter、Mermaid graph formatter、query memory、distribution，以及
  `explain(pipeline: true, json: true)` / `explain(pipeline: true, graph: true)`。
- benchmark runner 可用保存的 JSON baseline 做回归比较，默认没有 baseline 时仍只产出 summary。

### Phase 17: Column-Statistics-Driven Cardinality Estimation

状态：已完成 CBO 内部列级统计传播。connector metadata 采集到的 NDV 和 null fraction 不再只停留在
statistics contract，而是沿 logical plan 进入 selectivity 和 cardinality 估算。

已落地：

- `HeuristicCostModel` 内部同时传播 cost 和 column statistics；公开 `CostModel` 接口保持稳定。
- `filter` 按列 NDV 估算等值/非等值谓词，并将 null fraction 纳入比较谓词选择率；未知统计继续使用
  保守启发式，不伪造精度。
- `project` 和 `rename` 会同步裁剪或重命名列统计；`limit`、`filter` 等缩减行数的算子会收紧后续 NDV
  上界。
- `group` 和 `distinct` 优先使用列级 NDV 估算输出基数，nullable distinct 会计入独立 NULL 分组；
  缺列统计时仍回退平方根启发式。

验收：

- CBO 单测覆盖 SQLite 列统计驱动的等值、非等值、group、distinct、project + rename 估算。
- nullable SQLite fixture 验证 null fraction 会真实影响 predicate selectivity 和 distinct 基数。

### Phase 18: Runtime Join Planning and Graph Examples

状态：已把脚本级列键 join 接入 federated physical planner。此前 `plan::Join`、local hash join 和
三段 join pipeline DAG 已存在，但 universe `join()` 和 `join.inner/left/right/full()` 仍会先物化
lazy 输入，脚本查询无法保留真正的逻辑 join。现在两侧都有 lazy plan、`on` 为列键且没有自定义
`as` 时，会直接构造 `plan::Join`；predicate join 和自定义 `as` 继续走 eager fallback。

已落地：

- universe `join()` 和 join package 的列键入口共享 lazy plan 构造逻辑，保留 bucket 和 range
  metadata，并继续支持 `inner`、`left`、`right`、`full`。
- `LocalHashJoinOperator` 按 Flux table group instance 建索引和匹配，不再把不同 group 的 rows
  拉平；输出行恢复 `_group` 合并和重名列处理，与 eager fallback 的语义一致。
- `sqlite_pushdown_explain.flux` 增加 optimized logical、physical 和 pipeline Mermaid graph 输出，
  用户可以直接通过 CLI 查看图形化计划文本。

验收：

- runtime 覆盖脚本级 universe join 的 logical / physical / pipeline Mermaid graph，确认 pipeline
  中出现 `join-left`、`join-right` 和 `LocalHashJoinOperator`。
- runtime 覆盖 join package 列键路径保持 lazy，并能通过 physical executor 正确物化。
- runtime 覆盖 grouped local hash join 只匹配 group instance 相同的数据。

### Phase 19: Executable Connector/Memory CBO Alternatives

状态：已把 connector alternatives 从 explain 占位信息升级为真实可执行候选。RBO 继续负责判断合法
下推边界；当整条单源 unary pipeline 可以下推时，CBO 会额外枚举每一个 connector/memory 分界点，
为候选 plan 插入显式 `Materialize(reason="cbo connector/memory boundary")`，独立估价后选择最低成本
方案。

已落地：

- `PlanAlternative` 持有真实 logical plan，不再只记录 shape、名称和复用的展示 cost。
- full connector pushdown 使用 connector execution location 估价；memory suffix 候选使用显式
  materialize 边界，边界下方按 connector cost、上方按 memory cost 估算。
- rejected alternatives 使用具体名称，例如
  `connector_prefix_memory_suffix_after_Filter` 和
  `connector_prefix_memory_suffix_after_SourceScan`，可以直接定位切分位置。
- join build-side alternatives 也持有独立 clone plan，估价时不再临时修改共享 logical node。
- physical planner 的普通执行路径消费 Fast CBO 选中的 plan，为后续引入更丰富 cost factor 保留真实
  执行入口。

验收：

- CBO 单测验证 `limit(filter(scan))` 枚举 full connector、filter 后切分和 source scan 后切分三个
  真实候选，检查 `Materialize` 边界结构，并确认 full connector cost 更低。
- Fast CBO 无 connector statistics 时仍保留 RBO 优先行为，不为执行热路径增加 metadata 查询。

### Phase 20: Wrapped Join Pipeline DAG and Pushdown Plan Ownership

状态：已完成 unary wrapper 下的 join DAG 保留。此前 physical planner 只会拆分顶层 `Join`；
`project(join(...))` 或 `sort(project(join(...)))` 会退化成单 producer operator tree，左右 connector
scan 不再成为独立 pipeline。现在 planner 会沿 unary spine 定位被包装的 join，递归生成左右 producer
pipeline，再按原顺序恢复 wrapper。

已落地：

- streaming wrapper 下的 `project(join(...))` 生成 `join-left`、`join-right`、`main` 三段 DAG；
  `main` 从两个 exchange source 读取输入，再执行 local hash join 和 project。
- blocking wrapper 下的 `sort(project(join(...)))` 先拆出 `breaker-input-left`、
  `breaker-input-right`、`breaker-input`，最后由 `main` 执行 sort 和 output；join DAG 与 blocking
  boundary 可以组合，不再互相覆盖。
- CBO 会递归处理 unary wrapper 下的 join build-side 选择，而不是只检查 root join。
- `PushdownPlan` 对 `SourceScanSpec` 改为值语义持有。CBO clone plan 后，异步 scheduler 生命周期不再
  依赖临时 logical plan 中的 borrowed pointer。

验收：

- runtime 覆盖 `project(join(...))` 和 `sort(project(join(...)))` 的 pipeline 数量、依赖关系、
  operator 顺序和结果正确性。
- Flux 脚本级 Mermaid pipeline graph 覆盖 wrapped join，确认图中存在
  `breaker-input-left`、`breaker-input-right` 和 `LocalHashJoinOperator`。
- CBO 单测覆盖 unary wrapper 下的 join build-side 选择。
- RBO/runtime 回归覆盖 connector pushdown plan 的值语义生命周期。

### Phase 21: Cost-Aware Partitioned Local Hash Join

状态：已完成大输入 local hash join 的单机 hash repartition。此前 wrapped join 虽然已经保留左右
producer DAG，但两侧最终仍 gather 到一个 `LocalHashJoinOperator` driver；输入规模增大后，hash
build/probe 会重新串行化。现在 planner 会根据两侧 connector statistics 估算输入规模：小 join 或
统计未知时保持 gather，大 join 按目标分区行数选择 2 到 8 个 partition。

已落地：

- 左右 producer 使用 `HashPartitionExchangeSinkOperator`，按 Flux group key 和 join columns
  共同分区；相同 group instance、相同 join key 的 rows 必然进入同一个 local hash join driver。
- root join 会生成多个并行 driver，每个 driver 消费一对 partition buffer；`project(join(...))`
  等 partition-local wrapper 在各 partition 内继续执行。
- join 作为 blocking wrapper 上游 producer 时也会并行化。例如 `sort(project(join(...)))` 中，
  多个 join driver 先写入 gather buffer，再由 root sort driver 保持全局排序语义。
- nested join 输入需要再次分区时，会保留其 producer DAG，并显式增加 repartition producer；
  不会退回递归 operator tree。
- pipeline text、JSON、profile 和 Mermaid graph 会暴露 hash distribution、partition keys、
  partition 数和 join driver 数。

验收：

- runtime 覆盖小 SQLite join 保持 gather 三段 DAG，避免小查询支付 repartition 固定成本。
- runtime 覆盖大 memory join 生成 hash repartition producer 和多个 root join driver，验证结果、
  profile 与 Mermaid graph。
- runtime 覆盖大 join 位于 `sort(project(...))` 下时仍生成 partitioned join producer，并验证全局
  排序结果。
- runtime 覆盖大 nested join 的独立 repartition producer，确认嵌套 DAG 不会折叠。

### Phase 22: Hash Partition Skew Profile

状态：已补齐 partitioned exchange 的运行时分区统计。此前 pipeline plan 和 profile 能看到 hash
distribution contract 与 partition 数，但无法判断 rows 是否均衡；热点 join key 或 group key 只能
从整体耗时侧面猜测。现在每个 `HashPartitionExchangeSinkOperator` 会记录实际路由到各 partition 的
rows/bytes，并在共享 pipeline stats 中聚合。

已落地：

- 新增 `ExchangePartitionStats`，保持 plan distribution 与 runtime metrics 分离：前者描述规划意图，
  后者描述实际数据分布。
- 多 driver partition sink 只汇总各自真实路由计数，不读取共享 buffer 累计值，避免重复统计。
- `FormatExecutionProfile` 输出 `exchange_partition_stats`；`FormatExecutionProfileJson` 输出
  `exchangePartitionStats`，可供 CLI、UI 和自动化回归消费。

验收：

- runtime 覆盖普通 partitioned join 的 partition rows 总和与 bytes 非零统计。
- runtime 构造单热点 key join，确认 profile 能识别全部 rows 集中在一个 partition 的 skew。

### Phase 23: Partitioned Join and Accumulator Fusion

状态：已完成 partitioned join 与两阶段 accumulator planner 的组合。此前大 join 可以并行执行，但
`group(join(...))`、`distinct(join(...))` 和 `aggregate(group(join(...)))` 仍会先把 join 结果
gather，再由单 driver 执行 accumulator 或重新分区。现在 partial accumulator 会直接包住每个
partitioned local hash join driver，中间 gather 被移除。

已落地：

- 新增统一 `BuildParallelInputForPlan`：普通 source 返回 multi-split driver roots；大 wrapped join
  返回 partitioned local hash join driver roots 和左右 producer dependencies。
- grouped aggregate、root group、root distinct planner 只依赖统一 parallel input contract，不再
  各自理解 connector scan 或 join 的内部构造。
- partial accumulator pipeline 显式依赖左右 partitioned producer；join、partition-local wrapper
  和 partial accumulator 在同一 driver 内连续执行，再按 accumulator key contract 写入 final
  exchange。
- 小查询和统计未知查询继续保持原有 gather / single-stage fallback。

验收：

- runtime 覆盖 `group(project(join(...)))`，确认 DAG 只有左右 repartition producer、
  `group-partial` 和 final root。
- runtime 覆盖 `distinct(project(join(...)))`，确认 partial distinct 直接消费 join driver，并验证
  去重结果。
- runtime 覆盖 `count(group(project(join(...))))`，确认 partial aggregate 直接消费 join driver，
  并验证每组聚合结果。

### Phase 24: Stable Parameterized Split SQL

状态：已完成 SQL connector split range 的稳定参数化。普通 pushdown predicate、time range、
limit/offset 此前已经进入 `ParameterizedSql`；本阶段补齐 SQLite rowid split，使 base query
也可以携带 bind vector。SQLite 和 MySQL 的 split SQL text 现在都不再随 range bounds 改变。

已落地：

- `BuildParameterizedScanSql` 支持接收参数化 base query，并保证 base params 排在外层 scan
  params 之前，顺序与 SQL placeholder 一致。
- SQLite rowid split 从 literal range SQL 改成 `rowid >= ? AND rowid <= ?`，仍复用原有 SQLite
  prepare/bind 执行路径。
- MySQL range split 已经通过 `ScanRequest.predicates` 进入同一 parameterized builder；
  默认 server-side prepared 路径无需额外改动。
- literal SQL builder 继续保留给显式关闭 MySQL prepared statement 的 benchmark 对照路径。

验收：

- SQL builder 单测直接验证 base range、外层 predicate 和 limit 的 placeholder 与 bind 顺序。
- SQLite connector 单测继续覆盖 rowid multi-split page source 汇总结果。
- MySQL connector 集成测试继续覆盖默认 prepared range split 汇总结果。

### Phase 25: Cost-Aware Broadcast Join for Small Build Sides

状态：已完成小 build side inner join 的 skew-resistant 本地并行策略。此前大 join 统一对左右两侧按
group key 和 join key 做 hash repartition；当 probe side 存在热点 key 时，即使 build side 很小，
同一热点仍会集中到一个 join driver。现在 planner 在统计可用、inner join、build side 足够小且
probe/build 比例明显悬殊时，广播 build side，并 round-robin 分散 probe rows。
...
- broadcast 仅用于 `inner join`。left/right/full join 保持 hash repartition，避免复制 preserved
  build rows 后重复输出 unmatched rows。
- CBO 等值 join 基数估算使用 join key NDV 和 null fraction；many-to-one join 不再退化成
  `min(left_rows, right_rows)` 低估，outer join 会按 preserved side 保持行数下界。
- broadcast join driver 仍可直接被 group/distinct/aggregate partial accumulator 消费，不引入中间
  gather。

验收：

- CBO 单测覆盖 4:1 many-to-one inner/left join 基数估算。
- runtime 构造 8192:1 单热点 inner join，确认 probe rows 分散到全部 join driver，build row 广播到
  每个 partition，结果行数正确，Mermaid graph 暴露 round-robin 与 broadcast。
...
- `README.md`：用户入口和示例。
- `support-matrix.md`：真实支持状态。
- 本文档：阶段状态、设计调整和新增风险。
- 如有 benchmark，再同步 `benchmark/README.md`。

## Open Questions

- MySQL 默认 prepared statement 已进入热路径，split SQL text 也已稳定；是否进一步引入
  per-connection statement cache？当前 scan streaming 仍使用独立连接，cache 生命周期需要和后续
  pooled streaming 的 ASAN 问题一起评估。
...
- MySQL integration test 在当前 Bazel/CI 环境中如何稳定运行，是否把 benchmark fixture 自动化纳入
  CI nightly 而不是普通单测？
- CBO 已使用 connector metadata 中的行数和列统计；后续是否补充显式 analyze、采样和持久化统计？
- Page source 的 batch decode 和 string/value allocation 是否需要进一步进入统一 runtime allocator /
  memory pool？

## Recommended Next Step

下一轮继续收紧扫描主干，同时把已具备的 join/exchange 骨架推进到更完整的单机 federated optimizer：
...
- MySQL statement cache：在 pooled streaming 可安全启用后，基于已稳定的 SQL text + bind vector
