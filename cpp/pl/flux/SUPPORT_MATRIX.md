...
| structured result output | Partial | The CLI supports `--output-format human|csv|json` plus `--result <name>` for narrowing multi-result scripts, with `--annotated-csv` kept as a CSV alias; CSV preserves existing `result`/`table` columns when present and derives `#group` metadata from the runtime `_group` object, while JSON exposes named results and table metadata for scripting, but full official result-set fidelity is still incomplete |
