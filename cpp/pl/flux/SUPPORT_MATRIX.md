...
| builtin 注册表 / stdlib 执行    | 部分支持 | 默认加载的 universe 风格顶层 builtin 包括 `len`、`string`、`contains`、`sum`、`mean`、`min`、`max`、`columns`、`keys`、`findColumn`、`findRecord`、`explain`、`range`、`filter`、`map`、`limit`、`tail`、`keep`、`drop`、`rename`、`duplicate`、`set`、`reduce`、`sort`、`group`、`window`、`pivot`、`fill`、`elapsed`、`difference`、`derivative`、`distinct`、`count`、`spread`、`quantile`、`median`、`first`、`last`、`top`、`bottom`、`union`、顶层 `join()`、`aggregateWindow`、`yield`；显式 package 包含 `array.from`、`array.concat`、`array.filter`、`array.map`、`array.contains`、`array.reduce`、`array.any`、`array.all`、`csv.from`、`sqlite.from`、`mysql.from`、`date.*`、`dict.fromList`、`dict.get`、`dict.insert`、`dict.remove`、`regexp.*`、`strings.*`、`math.*`、`json.encode`、`runtime.version`、`system.time`、`types.isBool`、`types.isDuration`、`types.isFloat`、`types.isInt`、`types.isNumeric`、`types.isRegexp`、`types.isString`、`types.isTime`、`types.isType`、`types.isUInt`、`join.inner`、`join.left`、`join.right`、`join.full`；没有 universe 顶层数据源入口，所有数据源走 `array.from` / `csv.from` / `sqlite.from` / `mysql.from` 这类 provider package API；其中 `dict` 当前以对象承载可执行子集，`json.encode` 当前以 string 承载官方 bytes 返回语义，`sqlite.from` 当前支持 SQLite table scan 和 rowid multi-split，`mysql.from` 当前支持 Boost.MySQL table scan、保守 `ScanRequest` 下推和整型 range split，SQL provider 用户 API 不提供 `query` 入口，`explain` 当前输出 logical plan、optimized logical plan、physical plan、pushdown 状态、可编译 SQL provider plan 的 `ScanRequest` 摘要或 `<no plan>`；physical execution 已有 `ExecutionTask` / `Pipeline` / `Scheduler` / `Driver` / `Page` / typed operator 主干，blocking unary operator 可以形成 producer/root pipeline DAG，physical plan 会显式展示 `OutputSink`、connector scan、typed memory operator、materialize operator、operator 名和 RBO/CBO trace；`quantile(q:)` 现在支持单个数值或数值数组，并会在输出行上附带 `quantile` 列；顶层 `builtin` 声明可绑定已知 builtin 或占位 callable；未知 package 会绑定为 metadata-only object；但完整 Flux 标准库仍远未实现 |
...
| SQLite 输入                     | 部分支持 | `import "sqlite"`、`sqlite.from(path:, table:)` 可通过 SQLite C API 扫描表并物化为内存 `TableValue`；支持 null/int/float/text/blob-as-string 类型映射；connector 抽象、logical plan skeleton、materialization barrier 和 SQL provider `ScanRequest` 下推已启动，runtime pipeline 已能下推 SQLite 源之上的 `range/filter(simple)/keep/drop/rename/sort/limit/distinct` 线性前缀，以及简单 `group(columns:) \|> count/sum/mean/min/max(column:)` 聚合；scan/filter/project/range 支持 rowid multi-split，Top-N 支持 split 内 partial Top-N 加 root 全局 sort/limit，blocking unary operator 直接消费 Page；用户 API 不提供 `query` 入口，复杂 `filter(fn:)` 和跨源 join 下推仍未实现                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| MySQL 输入                      | 部分支持 | `import "mysql"`、`mysql.from(dsn:, table:)` 和 `mysql.from(host:, user:, password:, database:, table:, ?port:)` 可通过 Boost.MySQL 扫描表并物化为内存 `TableValue`；支持 `mysql://user:password@host[:port]/database` 与 `user:password@tcp(host[:port])/database` DSN；支持 null、signed/unsigned integer、float/double、string/blob、date/datetime/time 映射；runtime pipeline 已能下推 MySQL 源之上的 `range/filter(simple)/keep/drop/rename/sort/limit/distinct` 线性前缀，以及简单 `group(columns:) \|> count/sum/mean/min/max(column:)` 聚合；scan/filter/project/range 会基于主键或整型列做保守 range split，Top-N 支持 split 内 partial Top-N 加 root 全局 sort/limit，blocking unary operator 直接消费 Page；用户 API 不提供 `query` 入口，复杂 `filter(fn:)` 和跨源 join 下推仍未实现                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
