...
| SQL 输入                        | 部分支持 | `import "sql"`、`sql.from(driver: "sqlite", dsn:, query:)` 可通过 SQLite C API 执行查询并物化为内存 `TableValue`；支持 null/int/float/text/blob-as-string 类型映射；connector 抽象、logical plan skeleton、materialization barrier 和 SQLite connector 侧 `ScanRequest` 下推已启动，runtime pipeline 已能下推 SQLite 源之上的 `range/filter(simple)/keep/drop/rename/sort/limit/distinct` 线性前缀，以及简单 `group(columns:) |> count/sum/mean/min/max(column:)` 聚合；复杂 `filter(fn:)`、跨源 join 下推和 MySQL 仍未实现                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
- 已有：`csv`，包含 `csv.from`
- 已有：`sql`，包含 `sql.from`；当前只支持 SQLite query 物化，connector / plan / pushdown 见 `DATASOURCE_ARCHITECTURE.md`
