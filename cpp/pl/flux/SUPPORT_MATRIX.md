...
| SQLite 输入                     | 部分支持 | `import "sqlite"`、`sqlite.from(path:, table:)` 可通过 SQLite C API 扫描表并物化为内存 `TableValue`；支持 null/int/float/text/blob-as-string 类型映射；connector 抽象、logical plan skeleton、materialization barrier 和 SQLite connector 侧 `ScanRequest` 下推已启动，runtime pipeline 已能下推 SQLite 源之上的 `range/filter(simple)/keep/drop/rename/sort/limit/distinct` 线性前缀，以及简单 `group(columns:) |> count/sum/mean/min/max(column:)` 聚合；用户 API 不提供 `query` 入口，复杂 `filter(fn:)`、跨源 join 下推和 MySQL 仍未实现                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| MySQL 输入                      | 部分支持 | `import "mysql"`、`mysql.from(dsn:, table:)` 已注册为 provider package API 骨架，会校验参数并拒绝 `query`，但 connector 尚未实现，调用返回 `Unimplemented`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
- `array.from` / `array.concat` / `array.filter` / `array.map` / `array.contains` / `array.reduce` / `array.any` / `array.all` / `csv.from` / `sqlite.from` / `date.*` / `dict.*` / `regexp.*` / `strings.*` / `math.*` / `json.encode` / `runtime.version` / `system.time` / `types.*` / `join.*` 这类 package 入口已可用，但完整标准库接口仍远未实现
...
数据源入口不归入默认 universe：当前使用 `array.from`、`csv.from`、`sqlite.from`，后续 MySQL 等外部源也按 `mysql.from` 这类 provider package 形态扩展。

### 官方 package 对齐

- 已有：`array`，包含 `array.from`、`array.concat`、`array.filter`、`array.map`、`array.contains`、`array.reduce`、`array.any`、`array.all`
- 已有：`csv`，包含 `csv.from`
- 已有：`sqlite`，包含 `sqlite.from`；当前只支持 SQLite table scan，connector / plan / pushdown 见 `DATASOURCE_ARCHITECTURE.md`
- 骨架：`mysql`，包含 `mysql.from(dsn:, table:)`，当前 connector 尚未实现
