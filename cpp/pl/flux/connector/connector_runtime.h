...
class TableValuePageSource final : public ConnectorPageSource {
public:
    TableValuePageSource(TableValue table, size_t rows_per_page)
        : table_(std::move(table)), rows_per_page_(std::max<size_t>(1, rows_per_page)) {}

    absl::StatusOr<std::optional<ConnectorPage>> NextPage() override {
        if (next_chunk_ >= table_.tables.size()) {
            return std::nullopt;
        }

        const TableChunk& source = table_.tables[next_chunk_];
        TableChunk chunk;
        ...
        Value value = Value::table_stream(table_.bucket, std::move(chunks), table_.range_start,
                                          table_.range_stop, table_.result_name);
        value.as_table_mut().plan = table_.plan;
        value.as_table_mut().materialized = table_.materialized;
        return ConnectorPage{.table = value.as_table()};
    }
