...
auto cpWidth(char32_t cp) noexcept -> int {
    ...
    if ((cp >= 0x0300 && cp <= 0x036F) || // Combining Diacritical Marks
        ...
        (cp >= 0x0670 && cp <= 0x0670) || (cp >= 0x06D6 && cp <= 0x06ED) ||
        (cp >= 0x0711 && cp <= 0x0711) || (cp >= 0x0730 && cp <= 0x074A) ||
        (cp >= 0x07A6 && cp <= 0x07B0) || (cp >= 0x07EB && cp <= 0x07F3) ||
        (cp >= 0x0816 && cp <= 0x082D) || (cp >= 0x0859 && cp <= 0x085B) ||
        (cp >= 0x08D4 && cp <= 0x0903) || (cp >= 0x093A && cp <= 0x093C) || // Devanagari
        (cp >= 0x093E && cp <= 0x094F) || (cp >= 0x0951 && cp <= 0x0957) ||
        (cp >= 0x0962 && cp <= 0x0963) || (cp >= 0x0981 && cp <= 0x0983) || // Bengali
        (cp >= 0x09BC && cp <= 0x09BC) || (cp >= 0x09BE && cp <= 0x09CD) ||
        (cp >= 0x09D7 && cp <= 0x09D7) || (cp >= 0x09E2 && cp <= 0x09E3) ||
        (cp >= 0x0A01 && cp <= 0x0A03) || // Gurmukhi
        (cp >= 0x0A3C && cp <= 0x0A3C) || (cp >= 0x0A3E && cp <= 0x0A4D) ||
        (cp >= 0x0A51 && cp <= 0x0A51) || (cp >= 0x0A70 && cp <= 0x0A71) ||
        (cp >= 0x0A75 && cp <= 0x0A75) || (cp >= 0x0A81 && cp <= 0x0A83) || // Gujarati
        (cp >= 0x0ABC && cp <= 0x0ACD) || (cp >= 0x0AE2 && cp <= 0x0AE3) ||
        (cp >= 0x0B01 && cp <= 0x0B03) || // Oriya
        (cp >= 0x0B3C && cp <= 0x0B3C) || (cp >= 0x0B3E && cp <= 0x0B4D) ||
        (cp >= 0x0B56 && cp <= 0x0B57) || (cp >= 0x0B62 && cp <= 0x0B63) ||
        (cp >= 0x0B82 && cp <= 0x0B82) || // Tamil
        (cp >= 0x0BBE && cp <= 0x0BCD) || (cp >= 0x0BD7 && cp <= 0x0BD7) ||
        (cp >= 0x0C00 && cp <= 0x0C03) || // Telugu
        (cp >= 0x0C3E && cp <= 0x0C4D) || (cp >= 0x0C55 && cp <= 0x0C56) ||
        (cp >= 0x0C62 && cp <= 0x0C63) || (cp >= 0x0C81 && cp <= 0x0C83) || // Kannada
        (cp >= 0x0CBC && cp <= 0x0CCD) || (cp >= 0x0CD5 && cp <= 0x0CD6) ||
        (cp >= 0x0CE2 && cp <= 0x0CE3) || (cp >= 0x0D01 && cp <= 0x0D03) || // Malayalam
        (cp >= 0x0D3E && cp <= 0x0D4D) || (cp >= 0x0D57 && cp <= 0x0D57) ||
        (cp >= 0x0D62 && cp <= 0x0D63) || (cp >= 0x0D82 && cp <= 0x0D83) || // Sinhala
        (cp >= 0x0DCA && cp <= 0x0DDF) || (cp >= 0x0DF2 && cp <= 0x0DF3) ||
        (cp >= 0x0E31 && cp <= 0x0E3A) ||                                   // Thai
        (cp >= 0x0E47 && cp <= 0x0E4E) || (cp >= 0x0EB1 && cp <= 0x0EB9) || // Lao
        (cp >= 0x0EBB && cp <= 0x0EBC) || (cp >= 0x0EC8 && cp <= 0x0ECD) ||
        (cp >= 0x0F18 && cp <= 0x0F19) || // Tibetan
        (cp >= 0x0F35 && cp <= 0x0F35) || (cp >= 0x0F37 && cp <= 0x0F37) ||
        (cp >= 0x0F39 && cp <= 0x0F39) || (cp >= 0x0F3E && cp <= 0x0F3F) ||
        (cp >= 0x0F71 && cp <= 0x0F84) || (cp >= 0x0F86 && cp <= 0x0F87) ||
        (cp >= 0x0F8D && cp <= 0x0FBC) || (cp >= 0x0FC6 && cp <= 0x0FC6) ||
        (cp >= 0x102B && cp <= 0x103E) || // SE Asian
        (cp >= 0x1056 && cp <= 0x1064) || (cp >= 0x1067 && cp <= 0x106D) ||
        (cp >= 0x1071 && cp <= 0x1074) || (cp >= 0x1082 && cp <= 0x108D) ||
        (cp >= 0x108F && cp <= 0x108F) || (cp >= 0x109A && cp <= 0x109D) ||
        (cp >= 0x135D && cp <= 0x135F) || // Ethiopic
        (cp >= 0x1712 && cp <= 0x1714) || // Philippine scripts
        (cp >= 0x1732 && cp <= 0x1734) || (cp >= 0x1752 && cp <= 0x1753) ||
        (cp >= 0x1772 && cp <= 0x1773) || (cp >= 0x17B4 && cp <= 0x17D3) || // Khmer
        (cp >= 0x17DD && cp <= 0x17DD) || (cp >= 0x180B && cp <= 0x180D) || // Mongolian
        (cp >= 0x1885 && cp <= 0x1886) || (cp >= 0x18A9 && cp <= 0x18A9) ||
        (cp >= 0x1920 && cp <= 0x193B) || // Limbu
        (cp >= 0x1A17 && cp <= 0x1A1B) || // SE Asian
        (cp >= 0x1A55 && cp <= 0x1A7C) || (cp >= 0x1A7F && cp <= 0x1A7F) ||
        ...
        (cp >= 0x1BA1 && cp <= 0x1BAD) || (cp >= 0x1BE6 && cp <= 0x1BF3) || // Batak
        (cp >= 0x1C24 && cp <= 0x1C37) ||                                   // Lepcha
        (cp >= 0x1CD0 && cp <= 0x1CE8) ||                                   // Vedic
        (cp >= 0x1CED && cp <= 0x1CED) || (cp >= 0x1CF2 && cp <= 0x1CF9) ||
        ...
        (cp >= 0xA674 && cp <= 0xA67D) || (cp >= 0xA69E && cp <= 0xA69F) ||
        (cp >= 0xA6F0 && cp <= 0xA6F1) || (cp >= 0xA802 && cp <= 0xA802) || // Syloti Nagri
        (cp >= 0xA806 && cp <= 0xA806) || (cp >= 0xA80B && cp <= 0xA80B) ||
        (cp >= 0xA823 && cp <= 0xA827) || (cp >= 0xA880 && cp <= 0xA881) || // Saurashtra
        (cp >= 0xA8B4 && cp <= 0xA8C5) || (cp >= 0xA8E0 && cp <= 0xA8F1) || // Devanagari Extended
        (cp >= 0xA926 && cp <= 0xA92D) ||                                   // Kayah Li
        (cp >= 0xA947 && cp <= 0xA953) ||                                   // Rejang
        (cp >= 0xA980 && cp <= 0xA983) ||                                   // Javanese
        (cp >= 0xA9B3 && cp <= 0xA9C0) || (cp >= 0xA9E5 && cp <= 0xA9E5) || // Myanmar
        (cp >= 0xAA29 && cp <= 0xAA36) ||                                   // Cham
        (cp >= 0xAA43 && cp <= 0xAA43) || (cp >= 0xAA4C && cp <= 0xAA4D) ||
        (cp >= 0xAA7B && cp <= 0xAA7D) || // Myanmar
        (cp >= 0xAAB0 && cp <= 0xAAB0) || // Tai Viet
        (cp >= 0xAAB2 && cp <= 0xAAB4) || (cp >= 0xAAB7 && cp <= 0xAAB8) ||
        (cp >= 0xAABE && cp <= 0xAABF) || (cp >= 0xAAC1 && cp <= 0xAAC1) ||
        (cp >= 0xAAEB && cp <= 0xAAEF) ||                                   // Meetei Mayek
        (cp >= 0xAAF5 && cp <= 0xAAF6) || (cp >= 0xABE3 && cp <= 0xABED) || // Meetei Mayek Ext
...
void Table::printBorder(std::ostream& out, std::size_t total, char32_t ch) {
    ...
    if (w == 0 || total < 2)
        return; // guard: control char or empty table
    auto utf8 = toUtf8(ch);
