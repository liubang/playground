...
- **企业级权限体系**：不做细粒度 RBAC、SSO/OIDC、组织架构同步、配额计费、TLS 终结（内网部署或反向代理场景自行处理）。四种静态角色 + collection 白名单覆盖目标场景（§10）。
...
| **RerankClient**     | OpenAI 兼容 rerank API 客户端（内网 API 或本地模型），可选            |
...
- **用户词典**：Jieba-CPP 的词典为预编译 darts 二进制，无运行时用户词典接口。内部系统名、黑话（talos、onesql、"打满/抖动/泄漏"搭配）的扩展路径：(a) query 侧同义词表扩展（见下条，M1 采用）；(b) 离线重生成词典——上游 `tools/generate_dict.py` 支持导入自定义词表后重建词典二进制（M2 评估为词典运营工具）。
...
v4 的全局 `--embedding_endpoint` 演进为 collection settings 内配置（不同 collection 可用不同模型/维度/端点：KB 用本地 MLX BGE-M3，代码检索用内网 code-embedding）。`EmbeddingClient` 抽象与 `OpenAIEmbeddingClient` 实现原样复用，按 endpoint+model 实例化并池化。
...
`RerankClient` 抽象（`Rerank(query, docs []string) []float32`），首版实现对接 OpenAI 兼容 `/v1/rerank`（内网 API 若有）；MLX reranker（bge-reranker-v2-m3 的 MLX 移植）列为 M2 评估项。rerank 输入 = top-50 的 `title + content` 截断（512 token），输出重排序。rerank 对排序质量的贡献通常大于分词与参数调优（业界经验，以 §11 评测验证）。
...
| **M1** 中文 hybrid（~2–3 个周末） | jieba analyzer（Jieba-CPP，集成已完成）+ 内存倒排 + BM25 + ANN 并行 + RRF + 降级；rerank 钩子（接内网 API）；golden set 50 条 | loom KB 上线；eval：hybrid 的 Recall@5 显著高于 vector-only 与 BM25-only 基线 |
...
| 3 | 内网 embedding/rerank API 的可用性与延迟 | 本地 MLX 为默认兜底；降级矩阵保证 BM25-only 可用 |
