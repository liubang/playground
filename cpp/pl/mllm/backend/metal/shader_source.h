...
// Metal Shading Language source for the MVP kernels. Compiled at runtime via
// MTLCreateSystemDefaultDevice + newLibraryWithSource so no `metal` compiler
// binary or prebuilt .metallib is needed (SPEC §5.2, §2.2).
//
// All kernels operate on f32 buffers; the backend converts f16/Q8_0 inputs on
// upload and writes f32 outputs on download.
...
// RoPE
// In-place GPT-NeoX style interleaved rotation (matches CPU reference):
//   x'[2i]   = x[2i] * cos(p*theta_i) - x[2i+1] * sin(p*theta_i)
//   x'[2i+1] = x[2i] * sin(p*theta_i) + x[2i+1] * cos(p*theta_i)
//   theta_i  = freq_base^(-2i/head_dim)
// One thread per (2i, 2i+1) pair; q first (q_elems elements), then k.
...
// Attention
// Single-token causal attention with GQA (matches CPU reference):
//   out[h, d] = sum_j softmax(scale * q[h] . k[j, kv_head]) * v[j, kv_head, d]
// One thread per (head, dim); each thread re-computes its scores serially.
    ...
    uint gid [[thread_position_in_grid]])
{
    ...
    // Pass 2/3: softmax normalization + weighted sum of values.
    float sumexp = 0.0f;
    float acc = 0.0f;
    for (uint j = 0; j < seq_len; ++j) {
        const device float* kj = kb + (size_t)j * kv_stride;
...
//   { fp16 scale (2 bytes, little-endian), 32 x int8 qs }  == 34 bytes.
