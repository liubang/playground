...
class PatternDictEncoder : public PatternEncoder {
public:
    absl::Status encode(std::span<const uint64_t> values, std::string& output) override;
    PatternId pattern_id() const override { return PatternId::kDictionary; }
};

class PatternDictDecoder : public PatternDecoder {
public:
    PatternDictDecoder(std::span<const std::byte> data, size_t count);
