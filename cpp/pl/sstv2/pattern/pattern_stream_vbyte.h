...
class PatternStreamVByteEncoder : public PatternEncoder {
public:
    absl::Status encode(std::span<const uint64_t> values, std::string& output) override;
    PatternId pattern_id() const override { return PatternId::kStreamVByte; }
};

class PatternStreamVByteDecoder : public PatternDecoder {
public:
    PatternStreamVByteDecoder(std::span<const std::byte> data, size_t count);

    uint64_t get(size_t index) const override;
    void get_batch(size_t start, size_t count, uint64_t* dst) const override;
    bool supports_random_access() const override { return false; }
