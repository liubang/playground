...
TEST(CompressTest, BlockFlagRoundTrip) {
    const uint64_t flags = block::encode_block_flag(Codec::kZstd);
    EXPECT_NE(flags & block::block_flags::kPatternStore, 0u);
    EXPECT_EQ(flags & block::block_flags::kRowKeyBitmap, 0u);
    EXPECT_EQ(flags & block::block_flags::kCompressMask,
              static_cast<uint64_t>(Codec::kZstd) << block::block_flags::kCompressShift);
    EXPECT_EQ(block::decode_block_flag(flags), Codec::kZstd);
}
