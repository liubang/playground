...
TEST(Binary, constructor) {
  ...
  pl::tools::Binary b2("hello", 5);
  EXPECT_STREQ(b2.data(), "hello");
  EXPECT_EQ(5, b2.size());

  const std::string s = "hello";
  pl::tools::Binary b3(s);
  EXPECT_STREQ(b3.data(), "hello");
  EXPECT_EQ(5, b3.size());

  const char* cs = "hello";
  pl::tools::Binary b4(cs);
  EXPECT_STREQ(b4.data(), "hello");
  EXPECT_EQ(5, b4.size());

  pl::tools::Binary b5(b4);
  EXPECT_STREQ(b5.data(), "hello");
  EXPECT_EQ(5, b5.size());
}

TEST(Binary, clear) {
  pl::tools::Binary b1("hello", 5);
...
TEST(Binary, compare) {
  pl::tools::Binary b1("hello", 5);
  pl::tools::Binary b2("helloo", 6);
  pl::tools::Binary b3("hello", 5);
