...
TEST(Binary, constructor) {
  playground::cpp::tools::Binary b1;
  EXPECT_STREQ(b1.data(), "");
  EXPECT_EQ(0, b1.size());

  playground::cpp::tools::Binary b2("hello", 5);
  EXPECT_STREQ(b2.data(), "hello");
  EXPECT_EQ(5, b2.size());

  const std::string s = "hello";
  playground::cpp::tools::Binary b3(s);
  EXPECT_STREQ(b3.data(), "hello");
  EXPECT_EQ(5, b3.size());

  const char* cs = "hello";
  playground::cpp::tools::Binary b4(cs);
  EXPECT_STREQ(b4.data(), "hello");
  EXPECT_EQ(5, b4.size());

  playground::cpp::tools::Binary b5(b4);
  EXPECT_STREQ(b5.data(), "hello");
  EXPECT_EQ(5, b5.size());
}

TEST(Binary, clear) {
  playground::cpp::tools::Binary b1("hello", 5);
...
TEST(Binary, compare) {
  playground::cpp::tools::Binary b1("hello", 5);
  playground::cpp::tools::Binary b2("helloo", 6);
  playground::cpp::tools::Binary b3("hello", 5);
