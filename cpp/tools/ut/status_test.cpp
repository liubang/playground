...
TEST(Status, constructor) {
  auto ok = playground::cpp::tools::Status::NewOk("ok");
  EXPECT_TRUE(ok.isOk());
  ...
  auto not_found = playground::cpp::tools::Status::NewNotFound("xxx");
  ...
  auto corruption = playground::cpp::tools::Status::NewCorruption("xxx");
