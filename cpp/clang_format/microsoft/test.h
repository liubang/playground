...
{
  public:
    Foo(std::string attr1, std::string attr2, std::string attr3, std::string attr4,
        const std::unordered_map<std::string, std::string> &attr5, uint64_t attr6,
        const std::shared_ptr<std::string> &attr7)
        : attr1_(std::move(attr1)), attr2_(std::move(attr2)), attr3_(std::move(attr3)), attr4_(std::move(attr4)),
          attr5_(attr5), attr6_(attr6), attr7_(attr7)
    {
    }

    virtual ~Foo() = default;

    uint64_t attr6() const
    {
        return attr6_;
    }
    ...
    void test_pointer(const char *s)
    {
        ::printf("s = %s\n", s);
    }

    void test_cf(const std::string &s)
    {
        std::cout << s << '\n';
    }
...
{
    static_assert((std::is_integral<T>::value && !std::is_same<T, bool>::value) || std::is_floating_point<T>::value,
