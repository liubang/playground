...
TEST(meta, expression) {
  {
    ...
    playground::cpp::meta::BinaryExpression exp(5, 3.5, plus);
    auto res = exp();
    EXPECT_EQ(8.5, res);
  }
...

  {
    ...
    std::vector<int> x{1, 2, 3}, y{1, 1, 1}, z{2, 5, 3};
    auto plus = [](auto x, auto y) { return x + y; };
    auto expr = playground::cpp::meta::BinaryContainerExpression(
        playground::cpp::meta::BinaryContainerExpression(x, y, plus), z, plus);
