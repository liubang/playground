...
public:
    ...
    template<class F, class... Args> void register_handler(F&& f, Args&&... args)
    {
        using RetType = decltype(f(args...));
        auto task =
            std::make_shared<RetType>(std::bind(std::forward<F>(f), std::forward<Args>(args)...));
    ...
    void add_appender(const std::string& name, LogAppenderInterface::Ptr appender);
...
private:
    void write_log(LogLevel level, const char* file_name, const char* function_name,
