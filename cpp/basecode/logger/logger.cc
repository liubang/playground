                       ...
                       int32_t line_num, const char* fmt, va_list ap)
{
  ...
  for (const auto& appender : appenders_) { appender.second->append(prefix.data(), prefix.size()); }
}

void Logger::add_appender(const std::string& name, LogAppenderInterface::Ptr appender)
{
  std::unique_lock<std::mutex> lk(mutex_);
