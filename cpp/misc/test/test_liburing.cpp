...
int main(int argc, char *argv[]) {
    ...
    io_uring_prep_provide_buffers(
        sqe, pl::misc::liburing::bufs, pl::misc::liburing::MAX_MESSAGE_LEN,
        pl::misc::liburing::BUFFERS_COUNT, pl::misc::liburing::group_id, 0);
    ...
    for (;;) {
        ...
        io_uring_for_each_cqe(&ring, head, cqe) {
            ...
            if (cqe->res == -ENOBUFS) {
                ::fprintf(stdout, "bufs in automatic buffer selection empty, this should not "
                                  "happen...\n");
                ::fflush(stdout);
                return 1;
            }
            if (type == pl::misc::liburing::Status::PROV_BUF) {
                ...
                if (cqe->res <= 0) {
                    ...
                    pl::misc::liburing::add_socket_write(&ring, conn_i.fd, bid, bytes_read, 0);
                }
            } else if (type == pl::misc::liburing::Status::WRITE) {
                pl::misc::liburing::add_provide_buf(&ring, conn_i.bid,
                                                    pl::misc::liburing::group_id);
                pl::misc::liburing::add_socket_read(&ring, conn_i.fd, pl::misc::liburing::group_id,
