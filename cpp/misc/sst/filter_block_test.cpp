...
TEST(filter_block, build_and_read) {
  std::unique_ptr<pl::FilterPolicy> filter_policy_ptr(
      pl::newBloomFilterPolicy(64));
  pl::FilterBlockBuilder builder(
  ...
  pl::FilterBlockReader reader(filter_policy_ptr.get(),
                                                       filter);
