    ...
    std::vector<int> spiralOrder(const std::vector<std::vector<int>>& matrix)
    {
        ...
        for (;;) {
            ...
            case Direction::Right:
                if (ymin > ymax)
                    return ret;
                for (y = ymin; y <= ymax; ++y) { ret.push_back(matrix[xmin][y]); }
                xmin++;
                d = Direction::Down;
                break;
            case Direction::Down:
                if (xmin > xmax)
                    return ret;
                for (x = xmin; x <= xmax; ++x) { ret.push_back(matrix[x][ymax]); }
                ymax--;
                d = Direction::Left;
                break;
            case Direction::Left:
                if (ymin > ymax)
                    return ret;
                for (y = ymax; y >= ymin; --y) { ret.push_back(matrix[xmax][y]); }
                xmax--;
                d = Direction::Up;
                break;
            case Direction::Up:
                if (xmin > xmax)
                    return ret;
                for (x = xmax; x >= xmin; --x) { ret.push_back(matrix[x][ymin]); }
                ymin++;
                d = Direction::Right;
                break;
            }
