...
auto operator>>=(const Maybe<T>& ma, F&& f) {
  ...
  return std::visit(
      overloaded{
          [&](const Just<T>& v) -> R { return f(static_cast<T>(v)); },
...
auto show(const Maybe<T>& value) -> void {
  std::visit(
      overloaded{
          [](const Just<T>& v) { std::cout << "Just " << v << std::endl; },
...
int main(int argc, char* argv[]) {
  ...
  show((just(3) >>= [](int v) -> Maybe<double> {
         return {v * 1.5};
       }) >>= [](double v) -> Maybe<double> { return {v * 1.5}; });

  show((nothing() >>= [](int v) -> Maybe<double> {
         return {v * 1.5};
       }) >>= [](double v) -> Maybe<double> { return {v * 1.5}; });
