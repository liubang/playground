...
ctl() {
    local r=$1 c=$2
    shift 2
    "${COMPOSE[@]}" exec -T "$r" minivessel-control --address=127.0.0.1:9300 --command="$c" "$@"
}
status() { ctl "$1" status; }
log_status() { "${COMPOSE[@]}" exec -T replica-a minivessel-control --address=sharedlog:9200 --command=sharedlog-status; }
field() {
    local text=$1 key=$2
    sed -n "s/.*${key}=\([^ ]*\).*/\1/p" <<<"$text"
}
wait_value() {
    local r=$1 value=$2 lrsn=$3
    for _ in {1..80}; do
        local s
        s=$(status "$r" 2>/dev/null || true)
        [[ $s == *"value=$value "* && $s == *"applied_lrsn=$lrsn "* ]] && return 0
        sleep .25
    done
    status "$r"
    return 1
...
promote_until() {
    local r=$1 deadline=$((SECONDS + 12))
    while ((SECONDS < deadline)); do
        if ctl "$r" promote; then return 0; fi
        sleep .25
    done
    echo "timed out promoting $r" >&2
    return 1
}
assert_clean_cluster() {
    local log replica
    log=$(log_status)
    [[ $(field "$log" durable_lrsn) == 0 && $(field "$log" object_count) == 0 && $(field "$log" writer_epoch) == 0 ]]
    for replica in replica-a replica-b replica-c; do
        local s
        s=$(status "$replica")
        [[ $s == *'role=standby '* && $s == *'value=0 '* && $s == *'applied_lrsn=0 '* && $s == *'transitions=0'* ]]
    done
}
assert_services_healthy() {
    local service state health
    for service in mysql namenode datanode1 datanode2 datanode3 sharedlog replica-a replica-b replica-c; do
        state=$(${COMPOSE[@]} ps -q "$service" | xargs docker inspect -f '{{.State.Status}}')
        health=$(${COMPOSE[@]} ps -q "$service" | xargs docker inspect -f '{{if .State.Health}}{{.State.Health.Status}}{{else}}none{{end}}')
        [[ $state == running && $health == healthy ]] || {
            echo "$service is state=$state health=$health" >&2
            return 1
        }
    done
}
logs_on_error() {
    local rc=$?
    [[ -n ${stale_error:-} ]] && rm -f "$stale_error"
    if ((rc)); then
        echo "FAILED: preserving containers and printing logs"
        "${COMPOSE[@]}" ps
...
test)
    ...
    wait_value replica-b 7 "$cp_lrsn"
    wait_value replica-c 7 "$cp_lrsn"
    ...
    [[ ${#paths[@]} -eq 4 ]] || {
        echo "expected 4 immutable objects, found ${#paths[@]}" >&2
        exit 1
    }
    for path in "${paths[@]}"; do
        ...
        [[ $block_rows -gt 0 ]] || {
            echo "no committed 3/3 block row for $path" >&2
            exit 1
        }
        data_rows=$(grep -Ec '^[[:space:]]*[0-9]+' <<<"$blocks")
        [[ $block_rows -eq $data_rows ]] || {
            echo "found a non-committed or non-3/3 block row for $path" >&2
            exit 1
        }
    done
    ...
    new_epoch=$(field "$(status replica-b)" writer_epoch)
    ((new_epoch > old_epoch))

    echo '[4/8] prove stale A append is authority-fenced as ABORTED without catalog publication'
    before=$(log_status)
    before_lrsn=$(field "$before" durable_lrsn)
    before_count=$(field "$before" object_count)
    stale_error=$(mktemp)
    if ctl replica-a add --delta=100 --request_id=stale-a 2>"$stale_error"; then
        echo 'stale A unexpectedly accepted write'
        exit 1
    fi
    grep -q '^ABORTED:' "$stale_error"
    rm -f "$stale_error"
    stale_error=
    after=$(log_status)
    [[ $(field "$after" durable_lrsn) == "$before_lrsn" && $(field "$after" object_count) == "$before_count" ]]
    a_status=$(status replica-a)
    [[ $a_status == *'role=standby '* && $a_status == *'transitions=3'* ]]
    ...
    c_state=$(${COMPOSE[@]} ps -a -q replica-c | xargs docker inspect -f '{{.State.Status}}')
    [[ $c_state == exited ]]
    ...
    wait_value replica-a 22 "$l5"
    wait_value replica-b 22 "$l5"
    wait_value replica-c 22 "$l5"
...
all)
    ...
    ;;
*)
    echo "usage: $0 {all|build|start|test|down|reset}" >&2
    exit 2
    ;;
esac
