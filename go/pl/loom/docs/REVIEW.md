...
| R5  | anthropic vs openai provider                                                                  | `toolResultContent`/`messageText`/schema 解码/Stream 骨架/read-error 收尾大段重复，建议抽 `model/wireutil`                                                                                                             | ✅ 已处理（2026-09-05，抽 `model/wireutil`）                         |
...
| R9  | 多处                                                                                          | 死代码/死配置：`CanTerminate` 恒 true；`MaxParallelTools`/`MaxRepeatedActions` 无消费方；`ControllerStateFatal` 从未赋值；`Message.MarshalJSON` 无操作；`contentResult` 未使用；edit 包 `hashBytes`/`sha256Hex` 双份 | ✅ 已全部处理（`MaxParallelTools`/`MaxRepeatedActions` 已随 CONTEXT v3 裁剪；`ControllerStateFatal` 于 2026-09-05 删除） |
...
- **遗留**：R6（controller RPC 样板）、R7（pendingHub）与 R3 不同源，继续挂账；R5（provider 间重复）已于 2026-09-05 处理。
