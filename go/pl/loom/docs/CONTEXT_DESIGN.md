...
> 状态：v2 已实施（2026-07-29；一轮独立评审修正 2 blocker / 6 major / 14 minor 后落地，bazel 31/31 测试通过）
> 作者：liubang
> 日期：2026-07-29
...
4. **预算只计量稀缺资源本身，不用代理指标**。成本用 tokens、时间用 wall time 直接计量；**禁止用"回合数"这类代理指标惩罚工作量**。失控（死循环、无进展）用行为检测识别，不用预算识别。
...
预算维度只保留**稀缺资源本身**（时间、成本）与 context 压力；回合数/工具调用数不再作为预算维度（见 4.4.3）。
...
- wall_time/cost 档位 2 文案："预算即将耗尽（~X/Y，维度 Z）。请立即收敛，做好收尾准备。"
- 注入点沿用现状 `prepare()`（transcript pairing-safe 的唯一安全点）；wall_time/cost 档位判定前先 `touchWallTime()` 折叠已逝窗口（评审 m12）；
- 每个档位每个维度**每 prompt 只发一次**（Loop 每 prompt 重建，"每 run"语义即为"每 prompt"）；压缩成功后 occupancy 维度的档位重新武装，其余维度单调不重置；
...
`CheckRunaway` 命中硬上限（wall_time/cost）时**不再直接 terminate**，进入软着陆。完整状态机规定如下（评审 M1/M2/M3 修正）：
...
2. 模型产出**无工具调用**的可见回复后，`Terminate(OutcomeBudgetExhausted)`；
...
**最终保险丝**：`MaxWallTime`（默认 30min）与 `MaxEstimatedCostUSD`（配置定价后生效）保留为仅有的硬预算维度，命中时走 4.4.2 软着陆。

**语义边界（评审 M6，显式承认）**：wall_time 与 cost 均为 **per-prompt 窗口**（`ResetUsageForNewTurn` 在每个 prompt 边界重置），不是会话级预算。删除 MaxTurns 后的影响：turn 内无限往返（含 goal 自动续回合）被 30min 拦住 ✓；用户不在场时无自动续命通道 ✓；但**会话级从此无预算**，且未配置定价的用户成本维度失明（`CostInputUSDPerMTok` 零值禁用），单 prompt 内 30min × 200k 窗口的 token 燃烧无成本护栏——此前被 50 回合隐性兜住。这是有意的取舍：时间维度是用户最能直接感知的护栏；会话级累计预算（不随 prompt 重置，可复用 goal `TokensUsed` 的跨 prompt 累计先例）列为二期可选项（§9）。
...
| `internal/agent/run.go` | `shouldCompact` 废除估算判据；`maybeInjectBudgetNotice` → 全维度梯度提醒（含 `touchWallTime`）；循环顶部硬检查改为 `PhasePreparing` 判定 + `wrapUpPending` 抑制；软着陆状态机（含自动 deny、崩溃恢复）；压缩后 UI warning 钩子；预算耗尽 stale 文案（"raise via LOOM_MAX_*"，指向已删除机制）更新 |
| `internal/agent/runaway.go`（新增） | 失控行为检测：重复调用 / 连续失败（执行期）/ 无进展计数 |
| `internal/agent/goal.go` | goal wrap-up 并入统一软着陆状态机 |
| `internal/domain/limits.go` | 删除 `MaxTurns`/`MaxToolCalls`/`MaxParallelTools`/`MaxRepeatedActions`（死配置）字段；`Check()` 同步裁剪为 wall_time/cost/occupancy 维度（`CheckBudget` 软告警正式接入 4.4.1）；`CheckRunaway` 只保留 wall_time/cost；`DefaultLimits`：MaxToolOutputBytes=48KB |
...
| `internal/config` | 新增 `context:` / `runaway:` 配置节（见 §6）；删除 `limits.max_turns` 等 schema 字段 |
...
limits:
  max_wall_time: 30m           # per-prompt 时间保险丝
  max_cost_usd: 5.0            # per-prompt 成本上限（需配置定价后生效）
  max_tool_output_bytes: 49152
  # max_turns / max_tool_calls / max_parallel_tools / max_repeated_actions 已删除；
...
| `budget.wrapup_started`（新增） | `dimension`/`usage`/`limit`；崩溃恢复的重置依据 |
| `run.completed` | 软着陆路径的 `budget_exhausted` 必有一条最终助手消息先行 |
| trace（Langfuse） | 压缩 span 记录 trigger 与 occupancy；notice/wrap-up 注入记录为 event |

状态栏：`ctx:~N/W` 口径统一为 occupancy/Effective；告警色由 notice 档位驱动（弃用累计 input/MaxInputTokens 口径）。
...
3. **软着陆状态机**：wall_time/cost 硬命中 → 恰好一次 wrap-up → `budget_exhausted` 且有最终助手消息；非 `PhasePreparing` 命中时先完成 phase 迁移；wrap-up 中发起工具调用 → 自动 deny（不经审批）+ 配对完整 + 强制终止；**崩溃恢复**：`budget.wrapup_started` 后崩溃 → RecoverRun 重置位继续软着陆；
4. **梯度提醒**：各维度各档每 prompt 恰好一次；压缩后 occupancy 档重置、其他维度不重置；notice 档位低于 trigger 的生命周期走查（B1 回归：两档都发得出）；
5. **失控检测**：重复调用（含 prepare 失败、hash 口径）第 2 次警告第 3 次终止；连续失败只计执行期；只读调研任务 10 回合不误报无进展（M4 回归）；纯原地打转被提醒；
...
1. **会话级累计预算**：wall_time/cost 均为 per-prompt 窗口（评审 M6）。是否补一个不随 prompt 重置的会话级成本预算（复用 goal `TokensUsed` 的跨 prompt 累计先例）？二期评估。
