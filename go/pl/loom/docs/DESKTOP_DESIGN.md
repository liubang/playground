...
- 状态：Implemented v6（v6：①移除 Notification Center 镜像——审批请求 webview 页内已展示，系统横幅纯噪音，且 ad-hoc 签名下 TCC 通知授权随每次重打包失效、反复弹授权框；`notifications.go`/`notify_darwin.go` 删除，§3.5 行为表与架构图同步更新。②版本号改为构建时 stamp 的 `<打包日期>.<git短hash>`（如 `20260815.82f4e2a`）——bazel 侧 `tools/workspace_status.sh`（`--workspace_status_command`）产出键，go_binary x_defs 注入二进制、`tools/version_status.bzl` + info_plist genrule 注入 plist（`--config=desktop` 已带 `--stamp`）；`package_release.sh` 的 go build 用同源 `-ldflags -X` 注入；`internal/version` 的 VERSION 文件与 semver 假设退役（`go test` 下为 `dev`）。v5：macOS 平台集成与发布工程——版本号单源化（`internal/version`）、squircle 图标、TCC usage descriptions、单实例 GUI 提示、Finder 启动 workspace 选择、窗口几何持久化、release 四产物（macOS 双架构 DMG + Linux CLI deb）与 strip（-s -w），见 §3.5/§6.5。v4：R-B2 实锤——AssetServer 通道 ContentLength 恒为 -1 且 responseWriter 无 http.Flusher（SSE 不可能），前后端通道从「AssetServer 进程内挂载」改为「常驻 loopback 监听 + bootstrap 跳转」（原 §2.3 降级方案转正）；token 注入主通道随之回到 URL fragment。v3：token 注入定稿 meta 标签（已随通道更换退役）；R-B1 三项 Bazel 适配；.app 打包签名验证通过。v2：前后端通道更正为 AssetServer 挂载（后被 v4 取代）；分享复制点更正为 main.js）
- 日期：2026-08-08
