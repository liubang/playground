...
func TestReadSkillDefaultPath(t *testing.T) {
	root := t.TempDir()
	writeSkill(t, root, "pandora", "pandora", "line one\nline two\n")
...
func TestReadSkillSubPathAndPagination(t *testing.T) {
	root := t.TempDir()
	dir := writeSkill(t, root, "pandora", "pandora", "")
	var ref []string
	for i := 1; i <= 10; i++ {
...
func TestReadSkillUnknownNameListsAvailable(t *testing.T) {
	root := t.TempDir()
	writeSkill(t, root, "pandora", "pandora", "")
	writeSkill(t, root, "review", "review", "")
	tool := newTool(t, loadCatalog(t, root))

	msg := prepareErr(t, tool, map[string]any{"name": "missing"})
	if !strings.Contains(msg, "pandora") || !strings.Contains(msg, "review") {
...
func TestReadSkillRejectsEscapes(t *testing.T) {
	...
	for name, path := range cases {
		t.Run(name, func(t *testing.T) {
			prepareErr(t, tool, map[string]any{"name": "pandora", "path": path})
		})
	}
	...
	prepareErr(t, tool, map[string]any{"name": "pandora", "path": "references/evil.md"})
}
...
func TestReadSkillRejectsSensitiveAndBinaryAndLarge(t *testing.T) {
	...
	if msg := prepareErr(t, tool, map[string]any{"name": "pandora", "path": "scripts/.env"}); !strings.Contains(msg, "sensitive") {
		t.Fatalf(".env error = %q, want sensitive", msg)
	}
	if msg := executeErr(t, tool, prepare(t, tool, map[string]any{"name": "pandora", "path": "assets/logo.bin"})).Message; !strings.Contains(msg, "binary") {
		t.Fatalf("binary error = %q", msg)
	}
	if msg := prepareErr(t, tool, map[string]any{"name": "pandora", "path": "big.txt"}); !strings.Contains(msg, "256KB") {
...
func TestReadSkillPrepareIsDeterministicForFreshness(t *testing.T) {
	...
	tool := newTool(t, loadCatalog(t, root))
	call := newCall(t, map[string]any{"name": "pandora"})
...
func TestReadSkillExecuteFailsClosedOnCatalogDrift(t *testing.T) {
	rootA := t.TempDir()
	writeSkill(t, rootA, "pandora", "pandora", "body A")
	atomic := loadCatalog(t, rootA)
	tool := newTool(t, atomic)
	prepared := prepare(t, tool, map[string]any{"name": "pandora"})
	...
	prepared = prepare(t, tool, map[string]any{"name": "pandora"})
	rootB := t.TempDir()
	writeSkill(t, rootB, "pandora", "pandora", "body B")
...
func TestReadSkillExecuteRejectsForgedCalls(t *testing.T) {
	...
	call := newCall(t, map[string]any{"name": "pandora", "resolved_path": "/etc/passwd"})
