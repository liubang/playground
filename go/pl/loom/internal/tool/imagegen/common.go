...
// baseTool is the imagegen-local variant of the shared tool skeleton. Like
// webfetch it carries no path validator: generate_image touches no workspace
// paths, so its prepared-call fingerprint only binds the canonical arguments
// and the risk level.
