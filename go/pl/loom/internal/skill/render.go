...
- Placeholder convention: skills may use literal tokens in place of absolute paths — <work_dir> denotes the workspace root (see the environment snapshot's "Workspace root"), and <SKILL_DIR>/<skill_dir> denotes the directory containing that skill's SKILL.md (the catalog entry's file path, minus the filename). Resolve these tokens yourself when composing commands or file paths; they are never substituted mechanically.
