...
func TestLoaderDisabledFilter(t *testing.T) {
	ws, home := newEnv(t)
	writeSkill(t, filepath.Join(ws, ".loom", "skills", "pandora"), "pandora", "repo copy")
	writeSkill(t, filepath.Join(home, ".agents", "skills", "pandora"), "pandora", "user copy")
	writeSkill(t, filepath.Join(ws, ".loom", "skills", "keep"), "keep", "d")

	loader := NewLoader(ws, defaultUserRoots(home), nil)
	loader.SetDisabled([]string{"pandora"})
	...
	cat = loadAll(t, loader)
	if got := cat.Find("pandora"); got == nil || got.Description != "repo copy" {
		t.Fatalf("pandora = %+v, want repo copy after re-enable", got)
	}
}

func TestLoaderDeleteRemovesDirectory(t *testing.T) {
	ws := t.TempDir()
	dir := filepath.Join(ws, ".loom", "skills", "pandora")
	path := writeSkill(t, dir, "pandora", "d")
...
func TestLoaderDeleteSymlinkUnlinksOnly(t *testing.T) {
	ws := t.TempDir()
	realDir := filepath.Join(t.TempDir(), "real-pandora")
	path := writeSkill(t, realDir, "pandora", "d")
	root := filepath.Join(ws, ".loom", "skills")
	if err := os.MkdirAll(root, 0o755); err != nil {
		t.Fatal(err)
	}
