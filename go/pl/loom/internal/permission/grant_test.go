...
func TestValidateRuleGrant(t *testing.T) {
	...
	cases := []struct {
		...
		{"network full", Rule{ArgvPrefix: []string{"mycli"}, Decision: "allow", Grant: &RuleGrant{Network: "full"}}, ""},
	...
	r := Rule{ArgvPrefix: []string{"x"}, Decision: "allow", Grant: &RuleGrant{Write: []string{"~/.mycli/"}}}
	if err := validateRule(&r); err != nil {
		t.Fatal(err)
	}
	want := filepath.Join(home, ".mycli")
...
func TestLoadRuleSetsStripsProjectGrants(t *testing.T) {
	...
	writeRulesFile(t, user, "u.json", `{"rules":[
		{"argv_prefix":["mycli"],"decision":"allow","grant":{"network":"full"}}
	]}`)
...
func TestRememberUpgradesGrant(t *testing.T) {
	s := NewSessionRules()
	if _, ok := s.RememberRunCmd([]string{"mycli", "query"}, domain.ExecGrant{}); !ok {
		t.Fatal("remember failed")
	}
	if _, ok := s.RememberRunCmd([]string{"mycli", "query"}, domain.ExecGrant{NetworkFull: true}); !ok {
		t.Fatal("re-remember failed")
	}
	grant, ok := s.Match([]string{"mycli", "query", "submit"})
	...
	_, rule := set.Evaluate([]string{"mycli", "query"})
...
func TestParseRunCmdCall(t *testing.T) {
	info, ok := ParseRunCmdCall([]byte(`{"program":"mycli","args":["query"],"needs_network":true}`))
	if !ok || info.Escalated || !info.NeedsNetwork {
		t.Fatalf("info = %+v ok=%v", info, ok)
	}
	if len(info.Argv) != 2 || info.Argv[0] != "mycli" || info.Argv[1] != "query" {
