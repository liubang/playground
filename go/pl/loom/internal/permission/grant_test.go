...
func TestRememberUpgradesGrant(t *testing.T) {
	...
	dir := t.TempDir()
	if err := AppendRememberedRule(dir, []string{"talos"}, domain.ExecGrant{}); err != nil {
		t.Fatal(err)
	}
	if err := AppendRememberedRule(dir, []string{"talos"}, domain.ExecGrant{NetworkFull: true}); err != nil {
		t.Fatal(err)
	}
