...
func TestImportRuleFile(t *testing.T) {
	...
	content := fmt.Sprintf(`{
  "rules": [
    {"argv_prefix": ["myapp"], "decision": "allow", "justification": "custom: myapp needs network", "grant": {"network": "full"}, "match": ["myapp query"]},
  ...
  "domains": [
	...
	_, r1 := set.Evaluate([]string{"myapp", "query"})
	if r1.Justification != "custom: myapp needs network" {
	...
	defer store2.Close()
	if err := store2.RememberRule(ctx, []string{"myapp"}, domain.ExecGrant{}); err != nil {
		t.Fatal(err)
	}
	...
	_, r3 := s3.Evaluate([]string{"myapp", "query"})
