...
func TestUnlessDangerousGrantsDeclaredNetwork(t *testing.T) {
	...
	call.Call.Arguments = []byte(`{"program":"mycli","args":["query","submit"],"needs_network":true}`)
	v := d.Evaluate(call)
...
func TestUnlessDangerousSimpleShellAutoAllows(t *testing.T) {
	...
	call := runCmdPrepared(t, "sh", "-c", "mkdir -p .myapp_logs")
	v := d.Evaluate(call)
...
func TestUnlessDangerousCompoundShellAutoAllows(t *testing.T) {
	...
	call := runCmdPrepared(t, "sh", "-c", "mkdir -p .myapp_logs && echo created")
...
func TestEscalatedRequiresUnsandboxedGrant(t *testing.T) {
	rules := &RuleSet{rules: []Rule{{
		ArgvPrefix: []string{"node", "/skills/myapp-shim.js"},
	...
	v := p.Decider(ModeOnRequest).Evaluate(domain.PreparedCall{
		Call: domain.ToolCall{Name: "run_cmd", Arguments: []byte(
			`{"program":"node","args":["/skills/myapp-shim.js","env","production"],"sandbox_permissions":"require_escalated","justification":"writes ~/Library/Logs"}`,
		)},
		Risk: domain.R3,
	})
	if v.Decision != domain.DecisionAsk || !v.Grant.Unsandboxed {
...
func TestParseRunCmdCallUnwrapsSimpleShell(t *testing.T) {
	info, ok := ParseRunCmdCall([]byte(`{"program":"sh","args":["-c","mycli query submit --scene SKILL"]}`))
	if !ok || !info.ShellUnwrapped {
		t.Fatalf("simple sh -c must unwrap, info = %+v", info)
	}
	want := []string{"mycli", "query", "submit", "--scene", "SKILL"}
...
func TestRuleMatchesUnwrappedShellArgv(t *testing.T) {
	...
	p := Policy{Rules: rules}
	call := runCmdPrepared(t, "sh", "-c", "mycli query submit")
	call.Call.Arguments = []byte(`{"program":"sh","args":["-c","mycli query submit"],"needs_network":true}`)
	v := p.Decider(ModeUnlessDangerous).Evaluate(call)
	if v.Decision != domain.DecisionAllow || v.Source != SourceRule {
		t.Fatalf("simple sh -c mycli = %s (%s), want allow from rule", v.Decision, v.Source)
	}
}
