...
export class Composer {
  ...
  constructor({
    ...
    if (this.boxEl) {
      ...
      this.boxEl.addEventListener("dragleave", (e) => {
        if (!this.boxEl.contains(e.relatedTarget))
  ...
  async _addFiles(files) {
    if (!this.imagesEnabled) {
      if (files.length)
        this.onError(this.imagesDisabledReason || "当前模型不支持图片输入");
      return;
    }
    for (const f of files) {
      ...
      if (f.size > MAX_SOURCE_BYTES) {
        this.onError(
          `图片「${f.name || "剪贴板"}」超过 ${MAX_SOURCE_BYTES / 1024 / 1024}MB 上限`,
        );
        continue;
      }
      try {
  ...
  setReadOnly(readOnly) {
    ...
    if (this.attachBtn)
      this.attachBtn.disabled = readOnly || !this.imagesEnabled;
    this._applyState();
  }
