...
export class Composer {
  ...
  constructor({
    ...
    const submit = () => {
      ...
      onSubmit(
        text,
        this.attachments.map((a) => ({ media_type: a.mediaType, data: a.data }))
      )
    }
  ...
  async _fileToAttachment(file) {
    ...
    if (scale < 1) {
      ...
      blob = await new Promise((resolve, reject) =>
        canvas.toBlob((b) => (b ? resolve(b) : reject(new Error('encode failed'))), mediaType, 0.85)
      )
    }
    bitmap.close?.()
  ...
  _applyState() {
    ...
    if (this.focused) {
      this.ta.placeholder = this.running
        ...
        : 'Enter to send · Shift+Enter for newline · 粘贴/拖入图片'
    } else {
