...
export class Transcript {
  ...
  applySnapshot(snap) {
    ...
    const closeTurn = () => {
      ...
      attachAssistantActions(lastAssistant, {
        createdAt: lastTs,
        fb: this._fbOpts(lastRunId),
      });
      lastAssistant = null;
    };
    for (const m of snap.messages || []) {
      ...
      const flushText = () => {
        ...
        if (m.role === "user") {
          closeTurn();
          this._append(
            userBlock(text, createdAt, images, this.io.fetchArtifactURL),
          );
        } else {
        ...
        switch (p.kind) {
          ...
          case "tool_call":
            ...
            if (p.tool_call) {
              ...
              const diffText = diffForToolCall(
                p.tool_call.name,
    ...
    for (const pr of snap.pending_requests || []) {
      if (pr.kind === "approval" && pr.approval)
        this._addApprovalCard(pr.approval);
      else if (pr.kind === "question" && pr.question)
        this._addQuestionCard(pr.question);
    }
  ...
  handleEvent(evt) {
    ...
    switch (evt.kind) {
      ...
      case "model.text_delta":
        ...
        if (!this._pendingStreamTs && evt.time)
          this._pendingStreamTs = evt.time;
      ...
      case "approval.resolved":
        this._collapseApproval(
          p.approval_id,
          p.decision === "allow",
          p.actor || "another client",
        );
        this._showThinking();
        break;
      case "question.asked":
      ...
      case "subagent.started":
        this._append(
          noticeBlock(`subagent started: ${p.role || p.session_id || ""}`),
        );
        break;
      case "subagent.finished":
        this._append(
          noticeBlock(`subagent finished: ${p.role || p.session_id || ""}`),
        );
        break;
      default:
  ...
  _collapseQuestion(questionId, skipped) {
    ...
    card.el.replaceWith(
      noticeBlock(skipped ? "question skipped" : "question answered"),
    );
    this._maybeFollow();
  }
}
