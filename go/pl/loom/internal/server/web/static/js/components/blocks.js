...
export function streamBlock() {
  ...
  const render = () => {
    ...
    if (last && (last.tagName === "P" || last.tagName === "LI"))
  ...
  const schedule = () => {
    ...
    const wait = Math.max(
      0,
      STREAM_RENDER_MIN_INTERVAL_MS - (performance.now() - lastRender),
    );
...
export function toolBlock(payload, hooks = {}) {
  ...
  if (payload.target)
    head.appendChild(el("span", "tool-target mono", payload.target));
...
export function approvalCard(payload, { onResolve, diffEl }) {
  ...
  if (payload.description)
    card.appendChild(el("div", "desc", payload.description));
...
export function questionCard(payload, { onAnswer }) {
  ...
  for (const opt of q.options || []) {
    ...
    if (opt.description)
      span.appendChild(el("span", "desc", " — " + opt.description));
  ...
  skip.onclick = () =>
    onAnswer({ selected: [], custom_text: "", skipped: true });
...
export function renderPlanInto(panel, plan) {
  ...
  summary.appendChild(
    el("span", "plan-progress", `${done}/${items.length} done`),
  );
  ...
  const resolve =
    ...
    ((id, size) =>
      fetch(`/v1/artifacts/${encodeURIComponent(id)}?size=${size}`)
        .then((r) => {
          if (!r.ok)
            throw new Error(`artifact fetch failed (HTTP ${r.status})`);
          return r.blob();
        })
        .then((blob) => ({
...
export function artifactBlock(artifact, resolveURL) {
  ...
  const fail = () =>
    b.replaceChildren(el("div", "notice is-warn", "artifact 加载失败"));
  resolveArtifact(artifact, resolveURL).then(({ url, mediaType, blob }) => {
    ...
    if (type.startsWith("image/")) {
      ...
      img.onerror = () =>
        b.replaceChildren(el("div", "notice is-warn", "图片加载失败"));
      img.src = url;
      makeZoomable(img);
...
export function compactBlock(p) {
  ...
  if (details.length)
    wrap.appendChild(el("div", "compact-detail", details.join(" · ")));
  return wrap;
}
