...
function preserveUnmanaged(cfg, orig) {
  ...
  for (const path of PRESERVE_PATHS) {
    ...
    if (v !== undefined && getPath(cfg, path) === undefined)
      setPath(cfg, path, v);
  }
}
...
function makeControl(spec) {
  ...
  if (t === "select" || t === "tristate") {
    ...
    ctl.type =
      t === "password" ? "password" : t === "number" ? "number" : "text";
...
function fillScope(scopeEl, obj) {
  for (const ctl of ownControls(scopeEl))
    fillControl(ctl, getPath(obj, ctl.dataset.cfgKey));
}
        ...
        [
          {
            ...
            optionHints: {
              ...
              "on-request":
                "沙箱内命令、工作区内读写免审批；仅出沙箱提权/网络放宽/危险清单弹审批",
              "unless-dangerous":
                "黑名单模式：仅危险清单和出沙箱提权弹审批，needs_network 直接放网",
              never:
                "无人值守：沙箱内放行，提权与危险命令直接拒绝，永不阻塞等待审批",
            },
          },
        ],
      ],
...
export class SettingsPanel {
  ...
  async _load(manual) {
    ...
    try {
      ...
      this._msg(
        r.exists ? "" : "首次配置：请先在「模型」页添加至少一个 provider",
      );
  ...
  _renderTabs() {
    ...
    for (const t of TABS) {
      const b = el(
        "button",
        "settings-tab" + (t.id === this.activeTab ? " is-active" : ""),
      );
      b.type = "button";
  ...
  _fieldRow(spec, resolveRef) {
    const resolve =
      resolveRef || (spec.revealRef ? () => spec.revealRef : null);
  ...
  _renderProviders(body) {
    ...
    const add = el(
      "button",
      "btn btn-secondary btn-sm set-add",
      "+ 添加 provider",
    );
    add.type = "button";
    add.onclick = () => {
    ...
    for (const p of this.cfg.providers || [])
      list.appendChild(this._providerCard(p));
  }
  ...
  _providerCard(p) {
    ...
    for (const m of p.models || [])
      models.appendChild(this._modelCard(card, m));
  ...
  _modelCard(providerCard, m) {
    ...
    const head = el("div", "set-card-head");
    head.appendChild(
      this._rowSummary(() => this._openModel(providerCard, card)),
    );
  ...
  _refreshModelRow(card) {
    ...
    if (Array.isArray(obj.modalities) && obj.modalities.includes("image"))
      parts.push("多模态");
  ...
  _setProviderCrumb() {
    ...
    if (!card) return;
    const name =
      card.querySelector(":scope > .set-card-head .set-input").value.trim() ||
      "未命名";
    let text = "模型提供方 / " + name;
    if (refs.openModel) {
      const mn = refs.openModel
        .querySelector('[data-cfg-key="name"]')
        .value.trim();
      text += " / " + (mn || "未命名模型");
    }
  ...
  _collectProviders(cfg) {
    ...
    for (const card of refs.list.children) {
      ...
      const models = [];
      let skippedModels = 0;
      for (const mc of card.querySelector(
        ":scope > .set-card-body > .set-models",
      ).children) {
        const m = {};
        collectScope(mc, m);
  ...
  async _refreshMcpStatus() {
    ...
    for (const card of refs.list.children) {
      const name = card
        .querySelector(":scope > .set-card-head .set-input")
        .value.trim();
  ...
  _renderMcpTools(card) {
    ...
    if (!st.tools || !st.tools.length) {
      sec.appendChild(
        el("div", "set-hint", "该服务器未暴露工具（或已被白/黑名单全部过滤）"),
      );
      return;
    }
    const name = card
    ...
    for (const t of st.tools) {
      ...
      if (descText.startsWith(descPrefix))
        descText = descText.slice(descPrefix.length);
      const desc = el(
        "span",
  ...
  async _reconnectMcp(card) {
    ...
    try {
      ...
      if (status.connected) this.toast(`MCP 服务器 ${name} 已连接`, true);
        ...
        this.toast(`MCP 服务器 ${name} 连接失败: ${status.error || "unknown"}`);
    } catch (e) {
  ...
  _refreshMcpRow(card) {
    ...
    card.querySelector(":scope > .set-card-head .set-card-meta").textContent =
      this._mcpMeta(card);
  }
  ...
  _mcpMeta(card) {
    ...
    const srv = {};
    for (const group of card.querySelectorAll(
      ":scope > .set-card-body > .set-group",
    )) {
      if (
        group.dataset.transport === "common" ||
        group.dataset.transport === transport
      ) {
  ...
  async _loadEnvironment(force) {
    const refs = this._tabRefs.env;
    if (!refs || !refs.content || refs.loading || (refs.loaded && !force))
      return;
    refs.loading = true;
    if (refs.refreshBtn) {
    ...
    try {
      ...
      for (const t of r.tools || [])
        refs.content.appendChild(this._envToolRow(t));
      refs.content.appendChild(el("h4", "set-env-sub", "候选目录（按优先级）"));
      const dirs = r.dirs || [];
      if (!dirs.length)
        refs.content.appendChild(el("div", "set-hint", "（无候选目录）"));
      ...
      if (r.effective_path) {
        ...
        refs.content.appendChild(
          el("div", "set-hint mono set-env-path", r.effective_path),
        );
      }
    } catch (e) {
      refs.content.textContent = "";
      if (e.status !== 401) {
        refs.content.appendChild(
          this._loadingEl("环境报告不可用: " + e.message, true),
        );
  ...
  _envToolRow(t) {
    ...
    row.appendChild(
      el(
        "span",
        "env-val mono" + (t.found ? "" : " is-missing"),
        t.found ? t.path : "未检测到",
      ),
    );
    return row;
  ...
  _envDirRow(d) {
    const row = el(
      "div",
      "env-row" + (d.status === "missing" ? " is-dim" : ""),
    );
    row.appendChild(el("span", "env-val mono", d.path));
    const src =
      { config: "配置", static: "内置", glob: "通配" }[d.source] || d.source;
    const st =
  ...
  _skillGroup(g) {
    ...
    if (g.root)
      sec.appendChild(el("div", "set-hint mono set-skill-root", g.root));
  ...
  _skillRow(sk) {
    ...
    head.appendChild(
      el(
        "span",
        "skill-scope" + (sk.scope === "repo" ? " is-repo" : ""),
        sk.scope,
      ),
    );
    if (sk.disabled)
      head.appendChild(el("span", "skill-scope is-off skill-off", "已禁用"));
  ...
  async _toggleSkill(sk, btn) {
    ...
    try {
      ...
      this.toast(
        sk.disabled ? `已启用 ${sk.name}` : `已禁用 ${sk.name}（立即生效）`,
        true,
      );
      ...
      if (e.status !== 401)
        this.toast((sk.disabled ? "启用失败: " : "禁用失败: ") + e.message);
    } finally {
      btn.disabled = false;
    }
  }
  ...
  _rulePackRow(p) {
    ...
    const riskLabel =
      { low: "低风险", medium: "中风险", high: "高风险" }[p.risk] || p.risk;
    head.appendChild(
      el(
        "span",
        "skill-scope" + (p.risk === "high" ? " is-repo" : ""),
        riskLabel,
      ),
    );
    if (p.installed)
      head.appendChild(el("span", "skill-scope is-off", "已启用"));
    ...
    if (p.reason)
      sec.appendChild(el("div", "set-hint set-tip", "信任边界：" + p.reason));
    const cmds = el("div", "pack-cmds");
    for (const c of p.commands || [])
      cmds.appendChild(el("code", "pack-cmd mono", c));
  ...
  async _toggleRulePack(p, btn) {
    ...
    try {
      ...
      btn.disabled = false;
      if (e.status !== 401)
        this.toast((p.installed ? "停用失败: " : "启用失败: ") + e.message);
    }
  }
  ...
  _renderWorkspaces(body) {
    ...
    for (const ws of this.cfg.workspaces || [])
      list.appendChild(this._wsCard(ws));
    const add = el(
      "button",
      "btn btn-secondary btn-sm set-add",
      "+ 添加工作区",
  ...
  _wsCard(ws) {
    ...
    const head = el("div", "set-card-head");
    head.appendChild(
      makeControl({ key: "name", type: "text", ph: "显示名（可选）" }),
    );
  ...
  _collectAll(cfg) {
    ...
    if (this._skippedCards > 0) {
      this.toast(
        `${this._skippedCards} 张卡片因缺少必填字段（名称/根目录）未被保存`,
      );
  ...
  _firstInvalid() {
      ...
      for (const card of prefs.list.children) {
        ...
        if (!p.base_url) {
          return {
            ...
            el: card.querySelector(
              ':scope > .set-card-body [data-cfg-key="base_url"]',
            ),
          };
        }
        if (p.api_key && p.api_key_env) {
          return {
            ...
            tab: "providers",
            el: card.querySelector(
              ':scope > .set-card-body [data-cfg-key="api_key_env"]',
            ),
          };
        }
        let namedModels = 0;
    ...
    if (mrefs && mrefs.list) {
      for (const card of mrefs.list.children) {
        const nameCtl = card.querySelector(
          ":scope > .set-card-head .set-input",
        );
        const transport = card.querySelector(".set-transport").value;
        const srv = {};
        for (const group of card.querySelectorAll(
  ...
  _locate({ tab, el }) {
    ...
    if (card && prefs && prefs.list && prefs.list.contains(card)) {
      ...
      if (modelCard && prefs.openModel !== modelCard)
        this._openModel(providerCard, modelCard);
    }
    const mrefs = this._tabRefs.mcp;
  ...
  _applyMsg(resp) {
    if (resp.apply_error)
      return `已保存，但热应用失败（重启后生效）: ${resp.apply_error}`;
    const a = resp.applied;
    if (!a) return "已保存";
    const parts = [];
    if (a.immediate && a.immediate.length)
      parts.push("立即生效: " + a.immediate.join("、"));
    if (a.next_turn && a.next_turn.length)
      parts.push("下一轮生效: " + a.next_turn.join("、"));
    if (a.restart && a.restart.length)
      parts.push("重启后生效: " + a.restart.join("、"));
    return parts.length
      ? "已保存 — " + parts.join("；")
      : "已保存（配置无变化）";
  }
