...
function lcsDiff(oldLines, newLines) {
  ...
  let i = 0, j = 0;
  while (i < n && j < m) {
    if (oldLines[i] === newLines[j]) { ops.push({ kind: " ", line: oldLines[i] }); i++; j++; }
    else if (dp[i + 1][j] >= dp[i][j + 1]) { ops.push({ kind: "-", line: oldLines[i] }); i++; }
    else { ops.push({ kind: "+", line: newLines[j] }); j++; }
  }
...
const EXT_TO_LANG = {
  ...
  sh: "bash", bash: "bash", zsh: "bash", lua: "lua", pl: "perl", php: "php",
  r: "r", swift: "swift", json: "json", yml: "yaml", yaml: "yaml", toml: "ini", ini: "ini",
  sql: "sql", graphql: "graphql", md: "markdown", html: "xml", htm: "xml", xml: "xml",
  css: "css", scss: "scss", less: "less", mk: "makefile", makefile: "makefile", diff: "diff",
};
...
export function renderDiff(diffText) {
  ...
  for (const raw of (diffText || "").split("\n")) {
    ...
    let cls = "d-ctx", sign = " ", text = raw;
    if (raw.startsWith("+")) { cls = "d-add"; sign = "+"; text = raw.slice(1); adds++; }
    else if (raw.startsWith("-")) { cls = "d-del"; sign = "−"; text = raw.slice(1); dels++; }
    else if (raw.startsWith(" ")) { text = raw.slice(1); }
    else if (raw === "" && !sawContent) { continue; }
    const line = el("div", "d-line " + cls);
    line.appendChild(el("span", "d-sign", sign));
    const code = el("code");
    const html = highlightToHtml(text, lang);
    if (html) code.innerHTML = html; // sanitizeHtml 已过滤（markdown.js）
