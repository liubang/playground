...
func TestSkillRepoSkillFlow(t *testing.T) {
	...
	opt, _, err := app.WireSkills(registry, ws, 0, config.ResolvedSkills{Enabled: true}, filepath.Join(home, ".loom", "skills"), false, slog.Default())
	if err != nil || opt == nil {
...
func TestSkillsDisabledFlow(t *testing.T) {
	...
	registry := agent.NewToolRegistry()
	opt, _, err := app.WireSkills(registry, ws, 0, config.ResolvedSkills{Enabled: false}, filepath.Join(home, ".loom", "skills"), false, slog.Default())
	if err != nil {
		t.Fatal(err)
	}
	if opt != nil {
