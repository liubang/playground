#!/bin/bash
# Create a self-signed code-signing certificate for AuraShot and import
# it into the login keychain.
#
# Why: ad-hoc signatures change on every rebuild, so the TCC
# screen-recording grant is lost each time. A stable self-signed
# identity keeps the designated requirement constant across rebuilds —
# grant once, keep forever. Good enough for personal machines;
# distribution still needs an Apple Developer account.
#
# Idempotent: if the identity already exists in the keychain, do nothing.
#
# Usage:
#   bazel run //swift/pl/aurashot:make-signing-cert
#   AURASHOT_CERT_CN="My Cert Name" bazel run //swift/pl/aurashot:make-signing-cert
set -euo pipefail

CN="${AURASHOT_CERT_CN:-AuraShot Dev (liubang)}"
DIR="${AURASHOT_CERT_DIR:-$HOME/.config/aurashot/codesign}"
DAYS="${AURASHOT_CERT_DAYS:-3650}"
...
if ! openssl pkcs12 -export -legacy \
    -out aurashot-dev.p12 -inkey key.pem -in cert.pem -passout "pass:$PASS" 2>/dev/null; then
    openssl pkcs12 -export \
        -certpbe PBE-SHA1-3DES -keypbe PBE-SHA1-3DES -macalg sha1 \
        -out aurashot-dev.p12 -inkey key.pem -in cert.pem -passout "pass:$PASS"
fi

echo "==> importing into login keychain"
security import aurashot-dev.p12 \
...
Done. Next steps:
  ...
  AURASHOT_SIGN_IDENTITY="$CN" bazel run //swift/pl/aurashot:install

  # (optional) put it in your shell rc so you can omit it next time:
  export AURASHOT_SIGN_IDENTITY="$CN"

Certificate material kept in $DIR (key.pem / cert.pem are the source of
truth; aurashot-dev.p12 password: $PASS — only needed to re-import the p12).
EOF
