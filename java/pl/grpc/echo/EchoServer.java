    ...
    public void echo(EchoRequest req, StreamObserver<EchoResponse> observer) {
      ...
      observer.onNext(
          EchoResponse.newBuilder()
              ...
              .setOriginalTimestamp(req.getTimestampUs())
  ...
  static class StreamServiceImpl extends StreamServiceGrpc.StreamServiceImplBase {
    ...
    public void serverStream(ServerStreamRequest req, StreamObserver<StreamItem> observer) {
      ...
      if (!req.getPattern().isEmpty()) {
        try {
          pattern = Pattern.compile(req.getPattern(), Pattern.CASE_INSENSITIVE);
        } catch (PatternSyntaxException e) {
          observer.onError(
              Status.INVALID_ARGUMENT.withDescription("Invalid regex").asRuntimeException());
          return;
        }
      }
    ...
    public StreamObserver<EchoRequest> clientStream(StreamObserver<EchoSummary> observer) {
      return new StreamObserver<>() {
        ...
        public void onNext(EchoRequest req) {
          summary.addMessages(req.getMessage());
          count++;
        }
  ...
  public EchoServer(int port, String serverId) throws IOException {
    server =
        ServerBuilder.forPort(port)
            ...
            .addService(new StreamServiceImpl(serverId))
            .build()
            .start();
