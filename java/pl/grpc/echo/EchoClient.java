...
public class EchoClient {
  ...
  public void doServerStream(String pattern, int maxResponses) {
    ServerStreamRequest req =
        ServerStreamRequest.newBuilder().setPattern(pattern).setMaxResponses(maxResponses).build();
    try {
      Iterator<StreamItem> items = streamBlockingStub.serverStream(req);
      System.out.println(
          "[ServerStream] receiving items (pattern='"
              + pattern
              + "', limit="
              + maxResponses
              + "):");
  ...
  public void doClientStream(List<String> messages) throws InterruptedException {
    ...
    StreamObserver<EchoRequest> requestObserver =
        streamAsyncStub.clientStream(
            new StreamObserver<>() {
              @Override
              public void onNext(EchoSummary summary) {
                System.out.println(
                    "[ClientStream] summary: count="
