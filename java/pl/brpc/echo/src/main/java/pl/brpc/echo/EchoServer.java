...
/**
 * Starlight echo server — implements the 2 unary RPCs (Echo + HealthCheck) using the same proto
 * message types as the C++ brpc server.
 *
 * <p>Starlight exposes both brpc binary protocol and Spring MVC REST on the same port. This server
 * is configured programmatically (non-Spring).
 */
public class EchoServer {

  /**
   * The service implementation. NOTE: Java timestamps use System.currentTimeMillis()*1000 which has
   * millisecond (not microsecond) granularity. The field type (int64) is wire-compatible with the
   * C++ brpc server which uses true microseconds.
   */
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      System.out.println(
          "[Java Starlight EchoServer] Listening on "
              + host
              + ":"
              + port
              + " (server_id: java-starlight-server)");
